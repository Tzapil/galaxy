...
const { RES, BATCH, BUILDINGS } = loadData();
const HULLS = JSON.parse(readFileSync(join(HERE, 'hulls.json'), 'utf8')).hulls;
const MODULES = JSON.parse(readFileSync(join(HERE, 'modules.json'), 'utf8')).modules;
const techDoc = JSON.parse(readFileSync(join(HERE, 'techs.json'), 'utf8'));
...
for (const t of TECHS) {
  ...
  if (t.repeatable && !(t.costGrowth > 1)) errors.push(`Технология ${t.id}: повторяемая без роста стоимости`);

  for (const e of [...(t.effects ?? []), ...(t.effectPerLevel ?? [])]) {
    switch (e.type) {
      case 'unlockModule':
        if (!moduleIds.has(e.id)) errors.push(`Технология ${t.id}: открывает несуществующий модуль "${e.id}"`);
        if (unlockedModules.has(e.id)) errors.push(`Модуль "${e.id}" открывается дважды: ${unlockedModules.get(e.id)} и ${t.id}`);
        unlockedModules.set(e.id, t.id);
        break;
      case 'unlockHull':
        if (!hullIds.has(e.id)) errors.push(`Технология ${t.id}: открывает несуществующий корпус "${e.id}"`);
        if (unlockedHulls.has(e.id)) errors.push(`Корпус "${e.id}" открывается дважды: ${unlockedHulls.get(e.id)} и ${t.id}`);
        unlockedHulls.set(e.id, t.id);
        break;
      case 'unlockBuilding':
        if (!buildingIds.has(e.id)) errors.push(`Технология ${t.id}: открывает несуществующее здание "${e.id}"`);
        if (unlockedBuildings.has(e.id)) errors.push(`Здание "${e.id}" открывается дважды: ${unlockedBuildings.get(e.id)} и ${t.id}`);
        unlockedBuildings.set(e.id, t.id);
        break;
      case 'modifier': {
        const tgt = e.target ?? '';
        if (tgt.startsWith('recipe:') && !recipeIds.has(tgt.slice(7))) errors.push(`Технология ${t.id}: модификатор на несуществующий рецепт "${tgt}"`);
        if (tgt.startsWith('module:') && !moduleIds.has(tgt.slice(7))) errors.push(`Технология ${t.id}: модификатор на несуществующий модуль "${tgt}"`);
        if (tgt.startsWith('building:') && !buildingIds.has(tgt.slice(9))) errors.push(`Технология ${t.id}: модификатор на несуществующее здание "${tgt}"`);
        break;
      }
      case 'ability': break;
      default: errors.push(`Технология ${t.id}: неизвестный тип эффекта "${e.type}"`);
    }
  }
}

// обратная проверка: всё ли фазы 1 кем-то открывается
for (const m of MODULES) if (m.phase === 1 && !unlockedModules.has(m.id)) errors.push(`Модуль "${m.id}" (${m.name}) не открывается ни одной технологией`);
for (const h of HULLS) if (h.phase === 1 && !unlockedHulls.has(h.id)) errors.push(`Корпус "${h.id}" (${h.name}) не открывается ни одной технологией`);
for (const b of BUILDINGS) if (!unlockedBuildings.has(b.id)) errors.push(`Здание "${b.id}" (${b.name}) не открывается ни одной технологией`);

// сверка с полем tech в самих сущностях
for (const m of MODULES) {
  if (m.phase !== 1 || !m.tech) continue;
  const actual = unlockedModules.get(m.id);
  if (actual && actual !== m.tech) warnings.push(`Модуль ${m.id}: поле tech="${m.tech}", а открывает его "${actual}"`);
}
for (const h of HULLS) {
  if (h.phase !== 1 || !h.tech) continue;
  const actual = unlockedHulls.get(h.id);
  if (actual && actual !== h.tech) warnings.push(`Корпус ${h.id}: поле tech="${h.tech}", а открывает его "${actual}"`);
}
...
(function walk() {
  ...
  while (changed) {
    ...
    for (const t of TECHS) {
      ...
      if (deps.length && deps.every((d) => reachable.has(d))) { reachable.add(t.id); changed = true; }
    }
  }
})();
for (const t of TECHS) if (!reachable.has(t.id)) errors.push(`Технология ${t.id} недостижима от корня`);

// ---------------------------------------------------------------- 3. циклы бутстрапа

const startTech = T.get('start');
const startBuildings = new Set((startTech?.effects ?? []).filter((e) => e.type === 'unlockBuilding').map((e) => e.id));
  ...
  while (changed) {
    ...
    for (const r of BATCH) {
      ...
      for (const out of Object.keys(r.outputs)) {
        if (!startProducible.has(out)) { startProducible.add(out); changed = true; }
      }
    }
  }
}
for (const dataType of DATA_TYPES) {
  ...
  if (!startProducible.has(id)) {
    errors.push(`ЦИКЛ БУТСТРАПА: ${id} невозможно произвести стартовыми зданиями — наука не запустится`);
  }
}
// потребности населения тоже должны быть покрыты с нуля
for (const need of ['food', 'water', 'medicine', 'consumer_goods', 'fuel']) {
  if (!startProducible.has(need)) errors.push(`ЦИКЛ БУТСТРАПА: ${need} невозможно произвести стартовыми зданиями`);
}

// ---------------------------------------------------------------- анализ

const totalCost = (t) => Object.values(t.repeatable ? t.baseCost : (t.cost ?? {})).reduce((a, b) => a + b, 0);
...
console.log(line);
console.log(`Узлов:            ${TECHS.length}  (повторяемых: ${TECHS.filter((t) => t.repeatable).length}, фаза 2: ${TECHS.filter((t) => t.phase === 2).length})`);
console.log(`Веток:            ${BRANCHES.length}`);
console.log(`Открывает:        ${unlockedModules.size} модулей, ${unlockedHulls.size} корпусов, ${unlockedBuildings.size} зданий`);
...
console.log('');
console.log(`Производимо стартовыми зданиями: ${[...startProducible].filter((x) => x !== 'energy').sort().join(', ')}`);
console.log('');
...
console.log('ВЕТКИ');
console.log(line);
console.log('ветка'.padEnd(28) + 'узлов'.padStart(7) + 'повтор'.padStart(8) + 'физика'.padStart(9) + 'инженер'.padStart(9) + 'био'.padStart(8) + 'итого'.padStart(9));
for (const br of BRANCHES) {
  const nodes = TECHS.filter((t) => t.branch === br.id);
  const sum = { physics: 0, engineering: 0, bio: 0 };
  for (const n of nodes.filter((x) => !x.repeatable)) for (const [k, v] of Object.entries(n.cost ?? {})) sum[k] += v;
  console.log(
    br.name.padEnd(28) + String(nodes.length).padStart(7) + String(nodes.filter((x) => x.repeatable).length).padStart(8) +
    String(sum.physics).padStart(9) + String(sum.engineering).padStart(9) + String(sum.bio).padStart(8) +
    String(sum.physics + sum.engineering + sum.bio).padStart(9));
}
const grand = { physics: 0, engineering: 0, bio: 0 };
for (const n of TECHS.filter((x) => !x.repeatable && x.phase === 1)) for (const [k, v] of Object.entries(n.cost ?? {})) grand[k] += v;
const grandTotal = grand.physics + grand.engineering + grand.bio;
console.log('');
console.log(`Итого по фазе 1: физика ${grand.physics} (${(grand.physics / grandTotal * 100).toFixed(0)}%), ` +
...
if (Math.min(...share) < 0.15) console.log('   ← перекос: один из видов данных почти не востребован, соответствующие лаборатории будут не нужны');
else console.log('   ✓ все три вида данных востребованы (каждый не меньше 15% суммарной стоимости)');
console.log('');
...
console.log(line);
const goals = [
  ['Крейсер', 'hull_cruiser'], ['Линкор', 'hull_battleship'],
  ['Щит Мк3', 'shields_3'], ['Рельсотрон Мк3', 'kinetic_3'], ['Плазма Мк3', 'plasma_3'],
  ['Колонизация', 'colonization'], ['Терраформирование', 'terraforming_1'],
  ['Термояд', 'fusion_power'], ['Региональные столицы', 'regional_governance'],
  ['Протоколы автономии', 'autonomous_protocols'],
];
console.log('цель'.padEnd(26) + 'узлов'.padStart(7) + 'физика'.padStart(9) + 'инженер'.padStart(9) + 'био'.padStart(8) + 'итого'.padStart(9));
const goalCosts = new Map();
for (const [label, id] of goals) {
  if (!T.has(id)) { console.log(`${label}: цель "${id}" не найдена`); continue; }
  ...
  console.log(label.padEnd(26) + String(cl.size).padStart(7) + String(sum.physics).padStart(9) +
    String(sum.engineering).padStart(9) + String(sum.bio).padStart(8) + String(tot).padStart(9));
}
console.log('');
...
console.log(`Выработка одной лаборатории: ${DATA_TYPES.map((d) => `${d} ${labRate[d].toFixed(2)}/день`).join(', ')}`);
console.log('');
...
for (const maxTier of [1, 2, 3, 4]) {
  ...
  const mods = set.flatMap((t) => (t.effects ?? []).filter((e) => e.type === 'unlockModule').map((e) => e.id));
  const hulls = set.flatMap((t) => (t.effects ?? []).filter((e) => e.type === 'unlockHull').map((e) => e.id));
  const cost = set.reduce((a, t) => a + totalCost(t), 0);
  const years6 = Math.max(...DATA_TYPES.map((d) => set.reduce((a, t) => a + (costByType(t)[d] || 0), 0) / (labRate[d] * 6) / 365));
  console.log(`тир ≤ ${maxTier}: ${String(set.length).padStart(3)} технологий, ${String(mods.length).padStart(2)} модулей, ${hulls.length} корпусов, стоимость ${String(cost).padStart(6)}, ~${years6.toFixed(0)} лет при 6 лабораториях`);
}
console.log('');
...
console.log('технология'.padEnd(34) + [1, 5, 10, 20, 40].map((l) => `ур.${l}`.padStart(11)).join(''));
for (const t of rep) {
  const base = Object.values(t.baseCost).reduce((a, b) => a + b, 0);
  let row = t.name.slice(0, 32).padEnd(34);
  for (const lvl of [1, 5, 10, 20, 40]) row += Math.round(base * Math.pow(t.costGrowth, lvl - 1)).toLocaleString('ru-RU').padStart(11);
  console.log(row);
}
console.log('');
