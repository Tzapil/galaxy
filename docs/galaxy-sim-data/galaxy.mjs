...
const SYSTEM_COUNT = Number((process.argv.find((a) => a.startsWith('--systems=')) ?? '').slice(10)) || 7;
...
function makeSystem(id, name, isHome) {
  if (isHome) {
    return {
      ...
      res: new Map(), colony: true, buildings: [], pop: PKG.population.start,
      construction: [], satEma: new Map(), capitalSpend: new Map(),
    };
  }
  const bodies = [];
  ...
  for (let i = 0; i < n; i++) {
    ...
    let r = rng() * total, tpl = BODY_TEMPLATES[0];
    for (const t of BODY_TEMPLATES) { r -= t.w; if (r <= 0) { tpl = t; break; } }
    const features = [...tpl.features];
    // редкие жилы кучные: 20% тел их несут, и обычно только одну
    if (rng() < 0.20) features.push(pick(RARE));
    bodies.push({
      id: `${id}-${i}`, name: `${name}-${i + 1}`, type: tpl.type,
      slots: tpl.slotsRange[0] + Math.floor(rng() * (tpl.slotsRange[1] - tpl.slotsRange[0])),
      features, yield: 0.7 + rng() * 0.6,
      used: 0, energy: 0, energyCap: 0,
    });
  }
  return { id, name, bodies, res: new Map(), colony: false, buildings: [], pop: 0,
           construction: [], satEma: new Map(), capitalSpend: new Map() };
}

const NAMES = ['Дом', 'Кайрос', 'Вельта', 'Ор', 'Тессера', 'Гелиос', 'Нидр', 'Аркад', 'Мора',
               'Секст', 'Виндра', 'Тарн', 'Эол', 'Кассий', 'Реген', 'Ксант', 'Дельта', 'Ирис',
               'Ном', 'Веспер', 'Крон', 'Атрия', 'Селен', 'Фарос'];
const systems = Array.from({ length: SYSTEM_COUNT }, (_, i) => makeSystem(i, NAMES[i] ?? `Сис-${i}`, i === 0));
...
for (const g of gates) { adj[g.a].push({ to: g.b, days: g.days }); adj[g.b].push({ to: g.a, days: g.days }); }
...
for (let s = 0; s < SYSTEM_COUNT; s++) {
  dist[s][s] = 0; hops[s][s] = 0;
  const seen = new Set();
  while (seen.size < SYSTEM_COUNT) {
    let u = -1, best = Infinity;
    for (let i = 0; i < SYSTEM_COUNT; i++) if (!seen.has(i) && dist[s][i] < best) { best = dist[s][i]; u = i; }
    if (u < 0) break;
    seen.add(u);
    for (const e of adj[u]) if (dist[s][u] + e.days < dist[s][e.to]) {
...
function placeBuilding(sys, buildingId, bodyId) {
  ...
  sys.buildings.push({
    ...
    workers: b.recipe ? (R.get(b.recipe)?.workers ?? POWER.get(buildingId)?.workers ?? 0) : (b.workers ?? 0),
...
function makeShip(role, at) {
  const profile = role === 'hauler'
    ? { cargoCapacity: 1600, fuelCapacity: 220, fuelPerJump: 7 }
    : { cargoCapacity: 400, fuelCapacity: 320, fuelPerJump: 10 };
  ships.push({ id: ++shipSeq, role, at, job: null, ...profile, fuel: 0,
               cargo: new Map(), departAt: -1, arriveAt: -1, dest: at, foundHere: false });
}
...
function colonyTick(sys, d) {
  ...
  for (const b of sys.buildings) {
    ...
    if (!ok) { b.idleReason = 'нет топлива'; continue; }
  ...
  for (const body of sys.bodies) body.energy = Math.min(body.energyCap, body.energy + (prodEnergy.get(body.id) ?? 0));
  ...
  for (const b of sys.buildings) {
    ...
    const rec = R.get(b.recipe);
    if (!rec) continue;
    if (workers < b.workers) { b.idleReason = 'нет рабочих'; continue; }
    const body = sys.bodies.find((x) => x.id === b.body);
    const eNeed = rec.inputs.energy ?? 0;
    if (body.energy < eNeed) { b.idleReason = 'нет энергии'; continue; }
    const missing = Object.entries(rec.inputs).find(([id, q]) => id !== 'energy' && get(sys, id) < q);
  ...
  for (const c of sys.construction) if (c.doneAt === d) {
    placeBuilding(sys, c.bid, c.body);
    if (VERBOSE) state.events.push(`[${(d / 365).toFixed(1)}] ${sys.name}: построено ${B.get(c.bid).name}`);
  }
...
function supplyRates(sys) {
  const prod = new Map(), cons = demandRate(sys);
  for (const b of sys.buildings) {
    const rec = R.get(b.recipe);
...
function builderAI(sys, d) {
  ...
  if (starved) {
    for (const gen of ['fusion_plant', 'fission_plant', 'solar_array']) {
      ...
      sys.construction.push({ bid: gen, body: starved.id, doneAt: d + g.buildDays, why: 'энергия' });
      return;
    }
  }
  ...
  for (const res of RES.values()) {
    ...
    for (const src of colonies) {
      ...
      for (const dst of colonies) {
        ...
        state.jobs.push({ res: res.id, from: src.id, to: dst.id, qty, gain, score: gain / Math.max(1, travel) });
      }
    }
  }
...
function shipTick(d) {
  for (const sh of ships) {
    if (sh.arriveAt === d) {                                // прибытие
      sh.at = sh.dest;
      sh.arriveAt = -1;
      const sys = systems[sh.at];
      for (const [id, q] of sh.cargo) { add(sys, id, q); state.moved += q; }
      sh.cargo.clear();
      if (sh.role === 'colonizer' && sh.foundHere) { foundColony(systems[sh.at], d); sh.foundHere = false; }
      sh.job = null;
    }
    ...
    if (!departShip(sh, at, job.to, d)) { add(at, job.res, take); sh.cargo.clear(); }
  }
}
...
function foundColony(sys, d) {
  ...
  const seed = ['solar_array', 'solar_array', 'housing', 'mine', 'ice_drill', 'water_plant',
  ...
  for (const [id, q] of Object.entries({ food: 120, water: 120, medicine: 30, consumer_goods: 40,
                                         metal: 300, alloys: 120, polymers: 80, biomass: 60,
                                         electronics: 60, composites: 60, fuel: 220 })) {
    add(sys, id, q);
  }
  state.events.push(`[${(d / 365).toFixed(1)}] ОСНОВАНА КОЛОНИЯ: ${sys.name} (оценка ${colonyScore(sys).toFixed(0)})`);
}
...
function colonizationAI(d) {
  ...
  if (state.jobs.length > idleHaulers * 4 + 6) {
    ...
    if (get(src, 'hull_frames') >= 3 && get(src, 'thrusters') >= 2 && get(src, 'reactors') >= 1) {
      add(src, 'hull_frames', -3); add(src, 'thrusters', -2); add(src, 'reactors', -1);
  ...
  if (!idle) {
    ...
    if (get(src, 'hull_frames') >= 6 && get(src, 'life_support') >= 8 && faction.credits > 400) {
      add(src, 'hull_frames', -6); add(src, 'life_support', -8);
      faction.credits -= 400;
...
function researchTick(d) {
  for (const dt of DATA_TYPES) {
    for (const c of faction.colonies) {
      const sys = systems[c];
      const q = get(sys, `data_${dt}`);
      if (q > 0) { faction.data[dt] += q; add(sys, `data_${dt}`, -q); }
    }
  }
  const avail = TECHS.filter((t) => !t.repeatable && t.phase === 1 && !faction.researched.has(t.id)
...
const state = { day: 0, jobs: [], events: [], starved: new Map(), moved: 0, trips: 0, fuelBlocked: 0, builtHaulers: 0, history: [] };
...
console.log(line);
console.log(`Систем: ${SYSTEM_COUNT}, врат: ${gates.length} (средняя связность ${(2 * gates.length / SYSTEM_COUNT).toFixed(1)})`);
console.log('');
for (const s of systems) {
  const rare = new Set(s.bodies.flatMap((b) => b.features.filter((f) => RARE.includes(f))));
  console.log(`  ${s.name.padEnd(10)} тел ${String(s.bodies.length).padStart(2)}, слотов ${String(s.bodies.reduce((a, b) => a + b.slots, 0)).padStart(3)}` +
...
console.log('год'.padStart(5) + 'население'.padStart(12) + 'колоний'.padStart(9) + 'зданий'.padStart(8) +
  'технол.'.padStart(9) + 'кораблей'.padStart(10) + 'перевезено'.padStart(12) + 'разброс цен'.padStart(13));
for (const h of state.history) {
  if (h.year % Math.max(1, Math.round(YEARS / 15)) !== 0 && h.year !== 1) continue;
  console.log(String(h.year).padStart(5) + h.pop.toFixed(0).padStart(12) + String(h.colonies).padStart(9) +
    String(h.buildings).padStart(8) + String(h.techs).padStart(9) + String(h.ships).padStart(10) +
    Math.round(h.moved).toLocaleString('ru-RU').padStart(12) + h.spread.toFixed(2).padStart(13));
}
console.log('');
...
console.log(`Транспортов в строю:         ${ships.filter((s) => s.role === 'hauler').length} (построено за партию: ${state.builtHaulers})`);
console.log('');
...
for (const id of VITAL) {
  ...
  console.log(`${id.padEnd(18)} ${String(days).padStart(7)} колонио-дней (${(days / DAYS * 100).toFixed(1)}% от длины партии)`);
}
console.log('');
...
for (const c of faction.colonies) {
  ...
  console.log(`${s.name.padEnd(10)} население ${s.pop.toFixed(0).padStart(5)}  зданий ${String(s.buildings.length).padStart(3)}` +
...
if (VERBOSE) {
  ...
  console.log(line);
  for (const e of state.events.filter((x) => x.includes('КОЛОНИЯ') || x.includes('транспорт'))) console.log('  ' + e);
}
console.log(line);
