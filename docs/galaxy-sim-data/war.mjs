...
import { HERE, loadData, computeValues } from './values.mjs';

const { RES, BATCH, CONT, BUILDINGS } = loadData();
const VALUE = computeValues(RES, BATCH.reduce((m, r) => {
  for (const o of Object.keys(r.outputs)) { if (!m.has(o)) m.set(o, []); m.get(o).push(r); }
  return m;
}, new Map()));
...
const RARE_RES = { rare_earth_vein: 'rare_earth', crystal_vein: 'crystals', radioactive_vein: 'radioactives' };

const GROWTH_R = 0.00012, ENERGY_CAP = 260, EMPLOYMENT = PKG.population.employmentRate;
...
const NAMES = ['Кайрос', 'Вельта', 'Ор', 'Тессера', 'Гелиос', 'Нидр', 'Аркад', 'Мора', 'Секст',
  'Виндра', 'Тарн', 'Эол', 'Кассий', 'Реген', 'Ксант', 'Дельта', 'Ирис', 'Ном', 'Веспер', 'Крон',
  'Атрия', 'Селен', 'Фарос', 'Лид', 'Обер', 'Пирр', 'Сигма', 'Тавр', 'Улла', 'Фрея'];

function homeBodies() {
  return PKG.homeSystem.bodies.map((b) => ({ ...b, used: 0, energy: 0, energyCap: 0 }));
}
function randomBodies(id) {
  const out = [], n = 3 + Math.floor(rng() * 4), total = BODY_TPL.reduce((a, t) => a + t.w, 0);
  for (let i = 0; i < n; i++) {
    let r = rng() * total, tpl = BODY_TPL[0];
    for (const t of BODY_TPL) { r -= t.w; if (r <= 0) { tpl = t; break; } }
    const features = [...tpl.features];
    if (rng() < 0.22) features.push(pick(RARE));
    out.push({ id: `${id}-${i}`, type: tpl.type, features,
      slots: tpl.slots[0] + Math.floor(rng() * (tpl.slots[1] - tpl.slots[0])),
      yield: 0.7 + rng() * 0.6, used: 0, energy: 0, energyCap: 0 });
  }
  return out;
}
...
for (let i = 0; i < SYSTEM_COUNT; i++) {
  systems.push({
    ...
    bodies: null, res: new Map(), owner: null, buildings: [], pop: 0,
    construction: [], satEma: new Map(), capitalSpend: new Map(), foundedAt: -1,
  });
}

const gates = [], gateSet = new Set();
...
for (const g of gates) { adj[g.a].push({ to: g.b, g }); adj[g.b].push({ to: g.a, g }); }
...
function route(from, to, factionId) {
  ...
  const d = new Array(SYSTEM_COUNT).fill(Infinity), h = new Array(SYSTEM_COUNT).fill(Infinity);
  d[from] = 0; h[from] = 0;
  const seen = new Set();
  while (seen.size < SYSTEM_COUNT) {
    let u = -1, best = Infinity;
    for (let i = 0; i < SYSTEM_COUNT; i++) if (!seen.has(i) && d[i] < best) { best = d[i]; u = i; }
    if (u < 0) break;
    seen.add(u);
    for (const e of adj[u]) {
      if (e.g.blockadedBy !== null && e.g.blockadedBy !== factionId) continue;
      if (d[u] + e.g.days < d[e.to]) { d[e.to] = d[u] + e.g.days; h[e.to] = h[u] + 1; }
    }
  }
...
const rawDist = systems.map((_, s) => {
  const d = new Array(SYSTEM_COUNT).fill(Infinity); d[s] = 0;
  const seen = new Set();
  while (seen.size < SYSTEM_COUNT) {
    let u = -1, best = Infinity;
    for (let i = 0; i < SYSTEM_COUNT; i++) if (!seen.has(i) && d[i] < best) { best = d[i]; u = i; }
...
const FNAMES = ['Республика Векта', 'Синдикат Ор', 'Ковенант Тайра', 'Лига Мендо', 'Directorate Ксан', 'Дом Аврелий'];
...
for (let f = 0; f < FACTION_COUNT; f++) {
  factions.push({
    id: f, name: FNAMES[f % FNAMES.length], p: PERSONALITIES[f % PERSONALITIES.length],
    credits: PKG.treasury.credits, researched: new Set(PKG.technologies),
    data: { physics: 0, engineering: 0, bio: 0 }, colonies: [starts[f]],
    unlocked: new Set(), ships: [], warships: 0, warshipQuality: 1,
    relations: new Map(), warExhaustion: 0, capital: starts[f],
    seceded: false, alive: true,
  });
}
for (const f of factions) for (const g of factions) if (f !== g) f.relations.set(g.id, { war: false, opinion: 0, since: 0 });

function refreshUnlocked(f) {
  f.unlocked = new Set();
  for (const id of f.researched) for (const e of T.get(id)?.effects ?? []) if (e.type === 'unlockBuilding') f.unlocked.add(e.id);
}
...
function placeBuilding(sys, bid, bodyId) {
  const b = B.get(bid), body = sys.bodies.find((x) => x.id === bodyId);
  ...
  sys.buildings.push({ bid, body: bodyId, recipe: b.recipe,
    workers: b.recipe ? (R.get(b.recipe)?.workers ?? POWER.get(bid)?.workers ?? 0) : (b.workers ?? 0),
    busyUntil: -1, idleReason: '' });
  return true;
}
...
for (const f of factions) {
  ...
  s.bodies = homeBodies();
  s.owner = f.id; s.pop = PKG.population.start; s.foundedAt = 0;
...
function makeShip(f, role, at) {
  const p = role === 'hauler' ? { cargoCapacity: 1600, fuelCapacity: 220, fuelPerJump: 7 }
                              : { cargoCapacity: 400, fuelCapacity: 320, fuelPerJump: 10 };
  const sh = { id: ++shipSeq, f: f.id, role, at, ...p, fuel: 0, cargo: new Map(),
               arriveAt: -1, dest: at, foundHere: false };
  f.ships.push(sh);
  return sh;
}
for (const f of factions) {
...
function demandRate(sys) {
  ...
  for (const b of sys.buildings) {
    ...
    if (p) { for (const [id, q] of Object.entries(p.inputsPerDay ?? {})) m.set(id, (m.get(id) ?? 0) + q); continue; }
    const rec = R.get(b.recipe);
    if (!rec) continue;
    for (const [id, q] of Object.entries(rec.inputs)) if (id !== 'energy') m.set(id, (m.get(id) ?? 0) + q / rec.durationDays);
  }
  for (const [id, rate] of Object.entries(NEEDS.perThousandPopPerDay)) m.set(id, (m.get(id) ?? 0) + sys.pop * rate);
...
function colonyTick(sys, f, d) {
  ...
  for (const b of sys.buildings) {
    const p = POWER.get(b.bid);
    if (!p) continue;
    if (!Object.entries(p.inputsPerDay ?? {}).every(([id, q]) => get(sys, id) >= q)) { b.idleReason = 'нет топлива'; continue; }
    for (const [id, q] of Object.entries(p.inputsPerDay ?? {})) add(sys, id, -q);
    pe.set(b.body, (pe.get(b.body) ?? 0) + p.outputPerDay.energy);
  }
  for (const body of sys.bodies) body.energy = Math.min(body.energyCap, body.energy + (pe.get(body.id) ?? 0));

  for (const [id, rate] of Object.entries(NEEDS.perThousandPopPerDay)) {
    const need = sys.pop * rate, taken = Math.min(get(sys, id), need);
    add(sys, id, -taken);
  ...
  const growth = ema >= 0.98 ? 1 : ema >= 0.90 ? 0 : ema >= 0.60 ? -0.15 : -0.6;
  ...
  for (const b of sys.buildings) {
    ...
    const rec = R.get(b.recipe), y = sys.bodies.find((x) => x.id === b.body)?.yield ?? 1;
  ...
  for (const b of sys.buildings) {
    ...
    const rec = R.get(b.recipe);
    if (!rec || workers < b.workers) { if (rec) b.idleReason = 'нет рабочих'; continue; }
    const body = sys.bodies.find((x) => x.id === b.body), eN = rec.inputs.energy ?? 0;
    if (body.energy < eN) { b.idleReason = 'нет энергии'; continue; }
    const miss = Object.entries(rec.inputs).find(([id, q]) => id !== 'energy' && get(sys, id) < q);
    if (miss) { b.idleReason = `нет ${miss[0]}`; continue; }
    body.energy -= eN;
...
function supplyRates(sys, f) {
  const prod = new Map(), cons = new Map(demandRate(sys));
  for (const b of sys.buildings) {
    const rec = R.get(b.recipe);
    if (!rec || POWER.has(b.bid)) continue;
    const y = sys.bodies.find((x) => x.id === b.body)?.yield ?? 1;
    for (const [id, q] of Object.entries(rec.outputs)) prod.set(id, (prod.get(id) ?? 0) + (q * y) / rec.durationDays);
  }
...
function tryBuild(sys, f, bid) {
  ...
  for (const [id, q] of Object.entries(cost)) { add(sys, id, -q); sys.capitalSpend.set(id, (sys.capitalSpend.get(id) ?? 0) + q); }
...
function builderAI(sys, f, d) {
  ...
  const starved = sys.bodies.find((body) =>
  ...
  const crunch = sys.buildings.filter((b) => b.idleReason === 'нет рабочих').length > sys.buildings.length * 0.10;
  ...
  for (const def of defs) for (const rec of BATCH.filter((r) => r.outputs[def.id])) {
...
function demolishOne(sys, f) {
  ...
  for (const b of sys.buildings) {
    ...
    for (const [id, q] of Object.entries(rec.outputs)) {
      ...
      if (!flowOk && years < 5) { ok = false; break; }
  ...
  for (const res of RES.values()) {
    ...
    for (const src of cols) {
      ...
      for (const dst of cols) {
        ...
        f.jobs.push({ res: res.id, from: src.id, to: dst.id, qty, score: (pd - ps) * qty / Math.max(1, r.days) });
      }
    }
  }
...
function refuel(sh, sys) {
  ...
  if (take > 0) { add(sys, 'fuel', -take); sh.fuel += take; }
}
function depart(sh, from, toId, f, d) {
  const r = route(from.id, toId, f.id);
  if (!r) { state.blockadeBlocked++; return false; }
  refuel(sh, from);
  const need = sh.fuelPerJump * Math.max(1, r.hops);
  if (sh.fuel < need) { state.fuelBlocked++; return false; }
  sh.fuel -= need;
  sh.dest = toId; sh.arriveAt = d + Math.max(1, r.days);
  state.trips++;
  return true;
}
...
function shipTick(f, d) {
  for (const sh of f.ships) {
    if (sh.arriveAt === d) {
      sh.at = sh.dest; sh.arriveAt = -1;
      const sys = systems[sh.at];
      for (const [id, q] of sh.cargo) { add(sys, id, q); state.moved += q; }
      sh.cargo.clear();
      if (sh.role === 'colonizer' && sh.foundHere) { foundColony(sys, f, d); sh.foundHere = false; }
    }
    if (sh.arriveAt > 0 || sh.role !== 'hauler') continue;
    const at = systems[sh.at];
    const idx = f.jobs.findIndex((j) => j.from === sh.at);
    if (idx < 0) { const j = f.jobs[0]; if (j && j.from !== sh.at) depart(sh, at, j.from, f, d); continue; }
    const job = f.jobs.splice(idx, 1)[0];
    const res = RES.get(job.res);
    const take = Math.min(job.qty, get(at, job.res), Math.floor(sh.cargoCapacity / (res.unitVolume || 1)));
    if (take < 10) continue;
    add(at, job.res, -take);
    sh.cargo.set(job.res, take);
    if (!depart(sh, at, job.to, f, d)) { add(at, job.res, take); sh.cargo.clear(); }
  }
}
...
function foundColony(sys, f, d) {
  ...
  sys.owner = f.id; sys.pop = 12; sys.foundedAt = d;
  f.colonies.push(sys.id);
  for (const bid of ['solar_array', 'solar_array', 'housing', 'mine', 'ice_drill', 'water_plant',
                     'farm', 'food_plant', 'pharma_plant', 'consumer_plant', 'smelter']) {
    const b = B.get(bid);
    const body = sys.bodies.find((x) => x.used + b.slots <= x.slots
      && (!b.placement.requires || x.features.includes(b.placement.requires)));
    if (body) placeBuilding(sys, bid, body.id);
  }
  for (const [id, q] of Object.entries({ food: 120, water: 120, medicine: 30, consumer_goods: 40,
    metal: 300, alloys: 120, polymers: 80, biomass: 60, electronics: 60, composites: 60, fuel: 220 })) add(sys, id, q);
...
function treasuryTick(f, d) {
  if (d % 30 !== 0) return;
  let tax = 0;
  for (const c of f.colonies) tax += systems[c].pop * 0.020 * 30;
  const upkeep = (f.ships.length * 2.5 + f.warships * 14) * 30;
  f.credits += tax - upkeep;
  f.lastTax = tax / 30; f.lastUpkeep = upkeep / 30;
  if (f.credits < 0) {
    // Казна в минусе — флот распускается. Это и есть ограничитель размера флота (решение №14).
    if (f.warships > 0) { f.warships--; ev(d, `${f.name}: расформирован военный корабль. Причина: казна в минусе (${f.credits.toFixed(0)} кр.)`); }
...
function militaryTick(f, d) {
  ...
  const yard = f.colonies.map((c) => systems[c])
    .filter((s) => Object.entries(cost).every(([id, q]) => get(s, id) >= q))
    .sort((a, b) => get(b, 'hull_frames') - get(a, 'hull_frames'))[0];
  if (!yard) { f.shipyardBlocked = (f.shipyardBlocked ?? 0) + 1; return; }
  if (f.credits < 300) { f.creditBlocked = (f.creditBlocked ?? 0) + 1; return; }
  ...
  for (const o of others) {
    ...
    if (rel.war) {
      ...
      if (f.warExhaustion > 0.7 || losing) {
        rel.war = false; o.relations.get(f.id).war = false;
        f.warExhaustion = 0; o.warExhaustion = 0;
        for (const g of gates) if (g.blockadedBy === f.id || g.blockadedBy === o.id) g.blockadedBy = null;
        ev(d, `${f.name} заключает мир с ${o.name}. Причина: ${losing ? 'превосходство противника' : 'усталость от войны'}`);
      }
      continue;
    }
    // Решение о войне: нужда (MRP) × способность / цена, плюс характер (решение №15)
    const bn = bottleneck(f);
    const theyHave = o.colonies.some((c) => systems[c].bodies.some((b) =>
      b.features.some((ft) => RARE_RES[ft] === bn.res)));
    ...
    if (score > 1.5 && myStr > 3) {
      rel.war = true; o.relations.get(f.id).war = true;
      rel.since = d;
      const why = theyHave && bn.ratio < 0.8
        ? `дефицит ${bn.res} (обеспеченность ${(bn.ratio * 100).toFixed(0)}%), ближайший источник — территория противника`
        : hegemon > 1 ? 'противник слишком усилился, коалиция против гегемона' : 'благоприятный баланс сил';
...
function blockadeTick(f, d) {
  ...
  for (const g of gates) {
    ...
    const oa = systems[g.a].owner, ob = systems[g.b].owner;
...
function cohesionTick(f, d) {
  ...
  for (const c of [...f.colonies]) {
    ...
    const tension = distance * 0.55 + freshness * 14 + shortage * 16 + f.warExhaustion * 8
                  + Math.max(0, f.colonies.length - capacity) * 1.8;
    if (tension > 40 && rng() < 0.30) {
      f.colonies = f.colonies.filter((x) => x !== c);
      s.owner = null; s.pop = Math.max(8, s.pop * 0.6);
      state.secessions.push({ f: f.name, sys: s.name, day: d, tension });
      ev(d, `РАСКОЛ: ${s.name} откалывается от ${f.name}. Напряжение ${tension.toFixed(0)} ` +
...
function researchTick(f, d) {
  for (const dt of DATA_TYPES) for (const c of f.colonies) {
    const s = systems[c], q = get(s, `data_${dt}`);
    if (q > 0) { f.data[dt] += q; add(s, `data_${dt}`, -q); }
  }
  const avail = TECHS.filter((t) => !t.repeatable && t.phase === 1 && !f.researched.has(t.id)
  ...
  for (const t of avail.sort((a, b) => sum(a) - sum(b))) {
    if (DATA_TYPES.every((dt) => f.data[dt] >= (t.cost?.[dt] ?? 0))) {
      ...
      f.researched.add(t.id); refreshUnlocked(f);
      break;
    }
  }
}
...
function expansionTick(f, d) {
  ...
  if ((f.jobs?.length ?? 0) > idleH * 4 + 6) {
    ...
    if (cap.owner === f.id && get(cap, 'hull_frames') >= 3 && get(cap, 'thrusters') >= 2 && get(cap, 'reactors') >= 1 && f.credits > 150) {
      add(cap, 'hull_frames', -3); add(cap, 'thrusters', -2); add(cap, 'reactors', -1);
      f.credits -= 150; makeShip(f, 'hauler', f.capital);
    }
  }
  ...
  if (!idle) {
    const cap = systems[f.capital];
    if (cap.owner === f.id && get(cap, 'hull_frames') >= 6 && get(cap, 'life_support') >= 8 && f.credits > 400) {
      add(cap, 'hull_frames', -6); add(cap, 'life_support', -8);
      f.credits -= 400; makeShip(f, 'colonizer', f.capital);
    }
    return;
  }
...
const state = { day: 0, events: [], wars: [], secessions: [], moved: 0, trips: 0,
                fuelBlocked: 0, blockadeBlocked: 0, blockadeDays: 0, blockadeNoted: new Set(), history: [] };

for (let i = 0; i < DAYS; i++) {
  for (const f of factions) {
    if (!f.alive) continue;
    if (!f.colonies.length) { f.alive = false; ev(state.day, `${f.name} прекращает существование`); continue; }
  ...
  if (state.day % 365 === 0) {
    state.history.push({
      ...
      f: factions.map((f) => ({
        ...
        tech: f.researched.size, war: f.warships, cr: f.credits,
...
console.log(line);
console.log(`МНОГОФРАКЦИОННЫЙ ПРОГОН — ${YEARS} лет, ${SYSTEM_COUNT} систем, ${FACTION_COUNT} фракции, seed ${SEED}`);
console.log(line);
for (const f of factions) console.log(`  ${f.name.padEnd(20)} ${f.p.name.padEnd(14)} столица ${systems[f.capital].name}`);
console.log('');

console.log('год'.padStart(5) + factions.map((f) => `${f.name.split(' ')[1] ?? f.name}`.slice(0, 8).padStart(14)).join('') + 'блокад'.padStart(9));
console.log('     ' + factions.map(() => 'кол/нас/флот'.padStart(14)).join('') + '');
for (const h of state.history) {
  if (h.year % Math.max(1, Math.round(YEARS / 15)) !== 0 && h.year !== 1) continue;
  console.log(String(h.year).padStart(5) +
...
for (const w of state.wars.slice(0, 14)) console.log(`  [${w.day / 365 | 0} г.] ${w.from} → ${w.to}: ${w.why}`);
console.log('');
...
for (const s of state.secessions.slice(0, 12)) console.log(`  [${s.day / 365 | 0} г.] ${s.sys} откололась от ${s.f} (напряжение ${s.tension.toFixed(0)})`);
console.log('');
...
console.log(line);
console.log('фракция'.padEnd(20) + 'характер'.padEnd(15) + 'колоний'.padStart(9) + 'население'.padStart(11) +
  'технол.'.padStart(9) + 'флот'.padStart(7) + 'казна'.padStart(10) + 'верфь стоп'.padStart(12) + 'казна стоп'.padStart(12));
for (const f of factions) {
  const pop = f.colonies.reduce((a, c) => a + systems[c].pop, 0);
  console.log(f.name.padEnd(20) + f.p.name.padEnd(15) + String(f.colonies.length).padStart(9) +
    pop.toFixed(0).padStart(11) + String(f.researched.size).padStart(9) + String(f.warships).padStart(7) +
    f.credits.toFixed(0).padStart(10) + String(f.shipyardBlocked ?? 0).padStart(12) + String(f.creditBlocked ?? 0).padStart(12));
}
...
console.log('');
console.log(`Доля лидера: ${(lead.colonies.length / Math.max(1, totalCol) * 100).toFixed(0)}% колонизированных систем ` +
...
console.log(line);
const KEYB = ['hull_yard', 'weapon_works', 'armor_works', 'reactor_works', 'thruster_works',
              'lifesupport_works', 'alloy_works', 'electronics_plant', 'composite_plant', 'refinery'];
console.log('фракция'.padEnd(20) + KEYB.map((b) => B.get(b).name.slice(0, 9).padStart(11)).join(''));
for (const f of factions) {
  const cnt = new Map();
  for (const c of f.colonies) for (const b of systems[c].buildings) cnt.set(b.bid, (cnt.get(b.bid) ?? 0) + 1);
...
for (const f of factions) {
  const best = f.colonies.map((c) => systems[c]).sort((a, b) => get(b, 'hull_frames') - get(a, 'hull_frames'))[0];
  if (!best) continue;
  console.log(`  ${f.name.padEnd(20)} ${['hull_frames', 'weapons', 'armor', 'reactors', 'thrusters']
    .map((r) => `${r} ${get(best, r).toFixed(0)}`).join(', ')}`);
}
console.log('');
