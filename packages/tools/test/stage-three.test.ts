...
describe("Stage 3 galaxy generation", () => {
  ...
  it("fails impossible start spacing with a clear bounded-attempt error", () => {
    expect(() =>
      buildGeneratedGalaxyWorld(data, 1, {
        systemCount: 20,
        factionCount: 8,
        factionMinJumps: 50,
  ...
  it("clusters rare resources and keeps each rare resource in several clusters", () => {
    ...
    const clusteredCorrelation = neighborResourceCorrelation(
      clusteredPresence,
      clustered.edges,
      rareEarth
    );
    ...
    for (const resource of rareResourceIndices(data)) {
      expect(
        rareResourceClusterCount(clusteredPresence, clustered.edges, resource)
      ).toBeGreaterThanOrEqual(normalizeGalaxyParams({ systemCount: 500 }).rareResourceClusterMin);
    }
  }, 120_000);
});

function worldHash(world: StageOneWorld): string {
  return hashState({
    tick: 0,
    rngStreams: [],
