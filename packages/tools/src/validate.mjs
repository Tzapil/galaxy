...
import { REPORT_DIR, LABOR_VALUE, loadData, computeValues, makeExploder } from "./values.mjs";
...
for (const s of SINKS) {
  for (const id of s.consumes)
    if (!RES.has(id)) errors.push(`Сток ${s.id}: неизвестный ресурс "${id}"`);
}
...
for (const b of BUILDINGS) {
  if (b.recipe && !recipeIds.has(b.recipe))
    errors.push(`Здание ${b.id}: неизвестный рецепт "${b.recipe}"`);
  if (!(b.slots >= 1)) errors.push(`Здание ${b.id}: некорректное число слотов`);
  for (const id of Object.keys(b.buildCost ?? {})) {
    if (!RES.has(id))
      errors.push(`Здание ${b.id}: неизвестный ресурс в стоимости постройки "${id}"`);
  }
  if (!(b.buildDays > 0)) errors.push(`Здание ${b.id}: некорректный срок постройки`);
}
for (const r of BATCH) {
  if (!r.building) {
    errors.push(`Рецепт ${r.id}: не указано здание`);
    continue;
  }
  if (!buildingIds.has(r.building))
    errors.push(`Рецепт ${r.id}: ссылается на несуществующее здание "${r.building}"`);
  if (!buildingsWithRecipe.has(r.id))
    warnings.push(`Рецепт ${r.id} не привязан ни к одному зданию обратной ссылкой`);
}
for (const c of CONT) {
  if (c.kind === "consumption") continue;
  if (c.building && !buildingIds.has(c.building))
    errors.push(`Процесс ${c.id}: несуществующее здание "${c.building}"`);
}
...
for (const c of CONT) {
  for (const id of Object.keys({ ...(c.inputs ?? {}), ...(c.inputsPerDay ?? {}) }))
    consumedSomewhere.add(id);
...
for (const [id, r] of RES) {
  ...
  if (!consumedSomewhere.has(id))
    errors.push(`Ресурс "${id}" (${r.name}) нигде не потребляется — мёртвый узел`);
}
...
console.log(line);
console.log(
  `Ресурсов:            ${RES.size}  (фаза 1: ${[...RES.values()].filter((r) => r.phase === 1).length})`
);
...
console.log(
  `Материальных циклов: ${materialCycles.length}${materialCycles.length ? " ← ОШИБКА" : "  (ок)"}`
);
for (const c of materialCycles) console.log(`   ${c.join(" → ")}`);
const uniqEnergyCycles = [...new Set(energyCycles.map((c) => c.join(" → ")))];
console.log(
  `Циклов через энергию: ${uniqEnergyCycles.length}  (допустимы, разрешаются фиксированной точкой)`
);
...
console.log(
  "ресурс".padEnd(18) +
    "ур".padStart(3) +
    "глуб".padStart(6) +
    "стоим/ед".padStart(11) +
    "объём".padStart(8) +
    "стоим/объём".padStart(13) +
    "  профиль"
);
const rows = [...RES.values()]
  .filter((r) => r.phase === 1)
  .sort((a, b) => a.tier - b.tier || value.get(a.id) - value.get(b.id));
for (const r of rows) {
  const v = value.get(r.id);
  const dens = r.unitVolume > 0 ? v / r.unitVolume : NaN;
  const bar = Number.isFinite(dens)
    ? "█".repeat(Math.min(40, Math.round(Math.log10(Math.max(dens, 0.01)) * 12 + 12)))
    : "—";
  console.log(
    r.id.padEnd(18) +
      String(r.tier).padStart(3) +
      String(depth.get(r.id)).padStart(6) +
      v.toFixed(2).padStart(11) +
      String(r.unitVolume).padStart(8) +
      (Number.isFinite(dens) ? dens.toFixed(2) : "—").padStart(13) +
      "  " +
      bar
  );
}
...
for (const [raw, refined] of pairs) {
  ...
  console.log(
    `${raw.padEnd(14)} → ${refined.padEnd(16)} плотность стоимости растёт в ${ratio.toFixed(1)}× ${ratio > 1.5 ? "✓" : "← слабо, возить сырьё не наказывается"}`
  );
}
console.log("");
...
for (const c of CONT.filter((x) => x.kind === "power")) {
  ...
  console.log(
    c.id.padEnd(16) +
      out.toFixed(0).padStart(9) +
      (cost / out).toFixed(3).padStart(10) +
      out.toFixed(0).padStart(9) +
      vol.toFixed(2).padStart(9) +
      ((vol / out) * 100).toFixed(2).padStart(17)
  );
}
console.log("");
...
for (const tier of [1, 2, 3, 4, 5]) {
  const group = [...RES.values()].filter(
    (r) => r.phase === 1 && r.tier === tier && Number.isFinite(value.get(r.id))
  );
  ...
  console.log(
    `Уровень ${tier}: медиана ${median.toFixed(2)}, разброс ${vals[0].toFixed(2)}–${vals.at(-1).toFixed(2)}` +
      (flagged.length
        ? `  ← выбросы: ${flagged.map((r) => `${r.id} (${value.get(r.id).toFixed(1)}, ×${(value.get(r.id) / median).toFixed(1)})`).join(", ")}`
        : "  ✓")
  );
}
console.log("");
...
for (const r of rares) {
  ...
  console.log(
    `${r.id.padEnd(16)} ${value.get(r.id).toFixed(2).padStart(7)}  = ×${ratio.toFixed(1)} от обычного  ${ok ? "✓" : "← вне коридора 2–5×, редкость учитывается дважды"}`
  );
}
...
for (const [id, qty] of sortedRaw) {
  console.log(
    `   ${id.padEnd(16)} ${qty.toFixed(1).padStart(9)} ед.   ${RES.get(id).rarity === "rare" ? "← редкое" : ""}`
  );
}
...
for (const c of CONT.filter((x) => x.kind === "power")) {
  ...
  if (!entries.length) {
    console.log(`${c.id.padEnd(16)} ничего не потребляет`);
    continue;
  }
...
`;
writeFileSync(join(REPORT_DIR, "chain-graph.mmd"), mmd, "utf8");
console.log("Диаграмма графа записана в reports/chain-graph.mmd");
console.log(line);
