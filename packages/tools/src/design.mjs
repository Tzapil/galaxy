...
import { DATA_DIR, loadData, computeValues, bundleCost } from "./values.mjs";
...
const modulesDoc = JSON.parse(readFileSync(join(DATA_DIR, "modules.json"), "utf8"));
const MODULES = modulesDoc.modules;
const doctrinesDoc = JSON.parse(readFileSync(join(DATA_DIR, "doctrines.json"), "utf8"));
...
const availableIn = (era) =>
  MODULES.filter((m) => m.phase === 1 && ERA_UNLOCKS[era].mods.has(m.id));
...
function stats(d) {
  ...
  const s = {
    ...
    power: 0,
    crew: 0,
    crewCap: h.crewCapacity,
    shieldHp: 0,
    shieldRegen: 0,
    armorRating: 0,
    structure: h.structure,
    dmg: { long: 0, medium: 0, short: 0 },
    weapons: [],
    cargo: 0,
    scan: 0,
    fuelCap: h.baseFuel,
...
function feasible(d, s = stats(d)) {
  const h = HULL.get(d.hull);
  for (const t of ["weapon", "defense", "propulsion", "utility"])
    if (s.used[t] > h.slots[t]) return false;
...
const designCost = (d) =>
  hullCost.get(d.hull) + d.modules.reduce((a, id) => a + moduleCost.get(id), 0);
...
const computeEhp = (s) =>
  s.structure / (1 - armorReduction(s.armorRating)) +
...
const DEFAULT_ENEMY = { shieldFraction: 0.4, armorRating: 30 };
...
function score(d, doc, s = stats(d), enemy = DEFAULT_ENEMY) {
  ...
  for (const m of s.weapons) {
    effDps +=
      m.damage *
      (m.bands.includes(doc.preferredBand) ? 1 : SCORING.offBandPenalty) *
      threatFactor(m, enemy);
  }
...
const meetsRequirements = (d, doc, s = stats(d)) => {
  const r = doc.require ?? {};
  return (
    s.speed >= (r.minSpeed ?? 0) &&
    s.cargo >= (r.minCargo ?? 0) &&
    s.scan >= (r.minScan ?? 0) &&
    s.mining >= (r.minMining ?? 0) &&
    s.colonists >= (r.minColonists ?? 0) &&
    s.troops >= (r.minTroops ?? 0)
  );
};
...
function singleMoves(d, pool) {
  ...
  const out = [];
  for (const m of pool) {
    if (s.used[m.slot] < h.slots[m.slot])
      out.push({ kind: "add", mod: m.id, apply: (x) => ({ ...x, modules: [...x.modules, m.id] }) });
  }
  for (let i = 0; i < d.modules.length; i++) {
    ...
    for (const m of pool) {
      ...
      out.push({
        ...
        idx: i,
        mod: m.id,
        apply: (x) => ({ ...x, modules: x.modules.map((v, j) => (j === i ? m.id : v)) })
      });
    }
  }
  return out;
...
function designShip(hullId, doc, pool, costCap = Infinity, enemy = DEFAULT_ENEMY) {
  ...
  const reactors = pool
    .filter((m) => m.family === "reactor")
    .sort((a, b) => moduleCost.get(a.id) - moduleCost.get(b.id));
  ...
  for (let iter = 0; iter < 200; iter++) {
    ...
    if (!best) {
      ...
      outer: for (const t of targets.slice(0, 10)) {
        ...
        for (const en of singleMoves(cur, pool)) {
          ...
          if (!best || eff > best.eff) {
            best = { cand, eff, d, compound: [en, t.mv] };
          }
        }
        if (best) break outer;
      }
...
function printDesign(label, r) {
  if (!r) {
    console.log(`${label}: проект не найден`);
    return;
  }
  ...
  console.log(
    `   слоты  W ${s.used.weapon}/${h.slots.weapon}  D ${s.used.defense}/${h.slots.defense}  P ${s.used.propulsion}/${h.slots.propulsion}  U ${s.used.utility}/${h.slots.utility}`
  );
  console.log(
    `   энергия ${(-s.power).toFixed(0)} свободно   экипаж ${s.crew}/${s.crewCap}   масса ${s.mass}   скорость ${s.speed.toFixed(1)}`
  );
  console.log(
    `   щиты ${s.shieldHp} (+${s.shieldRegen}/р)   броня ${s.armorRating} (−${(armorReduction(s.armorRating) * 100).toFixed(0)}% урона)   структура ${s.structure}   EHP ${ehp}`
  );
  console.log(
    `   урон  дальняя ${s.dmg.long}  средняя ${s.dmg.medium}  ближняя ${s.dmg.short}` +
      (s.cargo ? `   трюм ${s.cargo}` : "") +
      (s.scan ? `   скан ${s.scan}` : "") +
      (s.mining ? `   добыча ${s.mining}` : "") +
      (s.troops ? `   десант ${s.troops}` : "") +
...
function makeShip(design, doc) {
  ...
  return {
    doc,
    design,
    name: HULL.get(design.hull).name,
    maxShield: s.shieldHp,
    shield: s.shieldHp,
    regen: s.shieldRegen,
    armor: s.armorRating,
    armorRed: armorReduction(s.armorRating),
    structure: s.structure,
    maxStructure: s.structure,
    speed: s.speed,
...
function bandStep(cur, fleetA, fleetB) {
  const wantA = fleetA[0].doc.preferredBand,
    wantB = fleetB[0].doc.preferredBand;
...
function moveBand(cur, target) {
  const i = BANDS.indexOf(cur),
    j = BANDS.indexOf(target);
  if (i === j) return cur;
...
function resolveCombat(fleetA, fleetB, rng, maxRounds = 40) {
  let band = "long";
  const initA = fleetA.length,
    initB = fleetB.length;
  for (let round = 1; round <= maxRounds; round++) {
    const A = fleetA.filter((s) => s.alive),
      B = fleetB.filter((s) => s.alive);
    ...
    if (lossA >= A[0].doc.withdrawAt && lossA > lossB)
      return {
        round,
        A: fleetA.filter((s) => s.alive).length,
        B: fleetB.filter((s) => s.alive).length,
        band,
        withdrew: "A",
        initA,
        initB
      };
    if (lossB >= B[0].doc.withdrawAt && lossB > lossA)
      return {
        round,
        A: fleetA.filter((s) => s.alive).length,
        B: fleetB.filter((s) => s.alive).length,
...
console.log(line);
{
  ...
  for (const [label, pool] of [
    ["все реакторы доступны", poolFull],
    ["реактор Мк3 не исследован", poolNoT3Reactor]
  ]) {
    const d = designShip("cruiser", doc, pool);
    const s = stats(d);
    console.log(`${label}:`);
    console.log(
      `   скорость ${s.speed.toFixed(1)}  щиты ${s.shieldHp}  броня ${s.armorRating}  урон(ср) ${s.dmg.medium}  стоимость ${designCost(d).toFixed(0)}`
    );
...
function profileOf(s) {
  const ehp = computeEhp(s);
  const shieldPart =
    (s.shieldHp * SCORING.shieldEhpFactor + s.shieldRegen * (SCORING.expectedBattleRounds ?? 6)) /
    Math.max(ehp, 1);
...
console.log(line);
{
  ...
  console.log(
    `   поздняя эпоха: ${nE} × ${HULL.get(elite.d.hull).name} по ${elite.cost.toFixed(0)} у.е.  (урон M ${elite.s.dmg.medium}, EHP ${Math.round(computeEhp(elite.s))})`
  );
  console.log(
    `   ранняя эпоха:  ${nC} × ${HULL.get(cheap.d.hull).name} по ${cheap.cost.toFixed(0)} у.е.  (урон M ${cheap.s.dmg.medium}, EHP ${Math.round(computeEhp(cheap.s))})`
  );
  ...
  console.log(
    `   → раунд ${res.round}, зона ${res.band}: поздних ${res.A}/${nE}, ранних ${res.B}/${nC}` +
...
console.log(line);
{
  ...
  for (const b of built) {
    console.log(
      `${b.name.padEnd(20)} ст.${b.cost.toFixed(0).padStart(5)}  ск.${b.s.speed.toFixed(1).padStart(5)}  щит ${String(b.s.shieldHp).padStart(4)}  бр.${String(b.s.armorRating).padStart(3)}  урон L/M/S ${b.s.dmg.long}/${b.s.dmg.medium}/${b.s.dmg.short}${b.ok ? "" : "  ← НЕВАЛИДЕН"}`
    );
  }
  console.log("");
...
console.log(line);
{
  ...
  console.log(
    `${HULL.get(raider.d.hull).name} (рейдер, скорость ${raider.s.speed.toFixed(1)}, урон L ${raider.s.dmg.long}) × ${nR}`
  );
  console.log(`против`);
  console.log(
    `${HULL.get(liner.d.hull).name} (линейный, скорость ${liner.s.speed.toFixed(1)}, урон M ${liner.s.dmg.medium}) × 3`
  );
  ...
  const r = resolveCombat(fa, fb, rng);
  console.log(
    `   → раунд ${r.round}, итоговая зона ${r.band}: рейдеров ${r.A}/${nR}, линейных ${r.B}/3` +
...
console.log(line);
{
  ...
  for (const docId of ["brawler", "line_battle", "escort"]) {
    ...
    for (const r of rows) {
      console.log(
        "   " +
          r.name.padEnd(23) +
          r.sc.toFixed(0).padStart(10) +
          r.c.toFixed(0).padStart(12) +
          r.eff.toFixed(0).padStart(17)
      );
    }
    if (chosen) {
      console.log(
        `   budget ${budget}: choose ${HULL.get(chosen.d.hull).name} x${chosen.affordableCount}, budgeted ${chosen.budgetedScore.toFixed(3)}, score/credit ${(chosen.scorePerCredit * 1000).toFixed(0)}/1000`
      );
    }
  }
  console.log("");
  ...
  console.log(
    "The table remains a score-per-credit diagnostic; the chosen line is the actual budget-aware result."
  );
}
...
console.log(line);
{
  ...
  for (const [label, enemy] of profiles) {
    ...
    console.log(
      `${label.padEnd(34)} оружие: ${guns.padEnd(16)} урон L/M/S ${s.dmg.long}/${s.dmg.medium}/${s.dmg.short}   стоимость ${designCost(d).toFixed(0)}`
    );
  }
  console.log("");
