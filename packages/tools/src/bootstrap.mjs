...
function placeBuilding(buildingId, bodyId) {
  ...
  if (
    !b.placement.on.includes(body.type === "asteroid" ? "planet" : body.type) &&
  ...
  state.buildings.push({
    ...
    recipe: b.recipe,
    workers: b.recipe
      ? (R.get(b.recipe)?.workers ?? p?.workers ?? b.workers ?? 0)
      : (b.workers ?? 0),
    busyUntil: -1,
    reserved: null,
    idleReason: ""
...
for (const s of PKG.buildings) {
  if (!placeBuilding(s.id, s.body))
    state.events.push(`[старт] не удалось разместить ${s.id} на ${s.body}`);
}
...
function tick() {
  ...
  const growthFactor = ema >= 0.98 ? 1 : ema >= 0.9 ? 0 : ema >= 0.6 ? -0.15 : -0.6;
  const popCap = currentPopCap();
  state.pop +=
    GROWTH_R * state.pop * (1 - state.pop / popCap) * growthFactor * (comfort < 0.5 ? 0.5 : 1);
  ...
  for (const b of state.buildings) {
    ...
    for (const [id, q] of Object.entries(rec.outputs))
      add(id, q * (bodyOf.get(b.body)?.yield ?? 1));
    b.busyUntil = -1;
    b.reserved = null;
  }
  ...
  for (const c of state.construction) {
    ...
    state.events.push(
      `[год ${(d / 365).toFixed(1)}] построено: ${B.get(c.bid).name} на ${bodyOf.get(c.body).name}`
    );
  }
...
function rates() {
  ...
  for (const b of state.buildings) {
    const p = POWER_BY_BUILDING.get(b.bid);
    if (p) {
      for (const [id, q] of Object.entries(p.inputsPerDay ?? {}))
        cons.set(id, (cons.get(id) ?? 0) + q);
      continue;
    }
    const rec = R.get(b.recipe);
    if (!rec) continue;
    for (const [id, q] of Object.entries(rec.outputs))
      prod.set(id, (prod.get(id) ?? 0) + q / rec.durationDays);
    for (const [id, q] of Object.entries(rec.inputs))
      if (id !== "energy") cons.set(id, (cons.get(id) ?? 0) + q / rec.durationDays);
  }
  for (const [id, rate] of Object.entries(NEEDS.perThousandPopPerDay))
    cons.set(id, (cons.get(id) ?? 0) + state.pop * rate);
...
function tryBuild(bid, why) {
  ...
  for (const [id, q] of Object.entries(cost)) {
    add(id, -q);
    state.capitalSpend.set(id, (state.capitalSpend.get(id) ?? 0) + q);
  }
...
function demolishForSpace() {
  ...
  if (cap > state.pop * 1.4) {
    ...
    if (h) {
      ...
      state.events.push(
        `[год ${(state.day / 365).toFixed(1)}] снесён жилой комплекс: ` +
  ...
  for (const b of state.buildings) {
    ...
    for (const [id, q] of Object.entries(rec.outputs)) {
      ...
      if (need <= 0) {
        redundant = false;
        break;
      } // выход никем не потребляется — иной случай
  ...
  state.events.push(
    `[год ${(state.day / 365).toFixed(1)}] снесено: ${B.get(best.b.bid).name} ` +
...
function builderAI(d) {
  ...
  const starvedBody = state.bodies.find(
    (body) =>
  ...
  const workerCrunch = idleForWorkers > state.buildings.length * 0.1;
  ...
  if (workerCrunch || (vitalOk && state.pop > 0.85 * currentPopCap())) {
    ...
    if (workerCrunch) {
      ...
      if (!state.crunchNoted) {
        ...
        state.events.push(
          `[год ${(d / 365).toFixed(1)}] нехватка рабочей силы, жильё поставить негде`
        );
      }
      return;
    }
  ...
  const vitalShort = deficits.find(
    (x) => ["food", "water", "medicine", "consumer_goods"].includes(x.id) && x.daysOfStock < 10
  );
  ...
  if (deficits.length && !state.saturatedAt) {
    state.saturatedAt = d;
    state.events.push(
      `[год ${(d / 365).toFixed(1)}] домашняя система насыщена: ` +
        `все ${state.bodies.reduce((a, b) => a + b.slots, 0)} слотов заняты, ` +
        `а дефицит остаётся (${deficits
          .slice(0, 3)
          .map((x) => x.id)
          .join(", ")}). Дальше только колонизация.`
    );
  }
}
...
console.log(
  "год".padStart(5) +
    ...
    "технол.".padStart(9) +
    "простой".padStart(9) +
    KEY.map((k) => k.slice(0, 7).padStart(9)).join("")
);
for (const h of state.history) {
  if (h.year % Math.max(1, Math.round(YEARS / 12)) !== 0 && h.year !== 1) continue;
  console.log(
    String(h.year).padStart(5) +
      h.pop.toFixed(1).padStart(12) +
      String(h.buildings).padStart(9) +
      String(h.techs).padStart(9) +
      String(h.idle).padStart(9) +
...
for (const id of allNeeds) {
  const s = state.starvedDays.get(id) ?? 0,
    t = state.tightDays.get(id) ?? 0;
  console.log(
    id.padEnd(18) +
      `${s} (${((s / DAYS) * 100).toFixed(1)}%)`.padStart(14) +
      `${t} (${((t / DAYS) * 100).toFixed(1)}%)`.padStart(19) +
      (s > 730 ? "  ← КРИТИЧНО" : "")
  );
}
console.log("");
...
for (const body of state.bodies) {
  ...
  console.log(
    `${body.name.padEnd(20)} слоты ${String(body.used).padStart(2)}/${String(body.slots).padEnd(3)} энергия ${body.energy.toFixed(0).padStart(4)}/${body.energyCap}`
  );
  if (counts.size)
    console.log(
      "   " + [...counts.entries()].map(([id, n]) => `${n}× ${B.get(id).name}`).join(", ")
    );
}
console.log("");
...
if (state.saturatedAt) {
  console.log(
    `Домашняя система насыщена на ${(state.saturatedAt / 365).toFixed(1)} году — дальше требуется колонизация.`
  );
  console.log("");
}
...
const checks = [
  [
    `Население не падало ниже ${C.populationNeverBelow}`,
    minPop >= C.populationNeverBelow,
    `минимум ${minPop.toFixed(1)}`
  ],
  [
    `Наука пошла до ${C.allThreeDataFlowingByYear} года`,
    dataYear && dataYear.year <= C.allThreeDataFlowingByYear,
    dataYear ? `первая технология к ${dataYear.year} году` : "ни одной технологии"
  ],
  [
    `Население выросло в ${C.populationMultipleByYear50}× к ${Math.min(50, YEARS)} году`,
    y50.pop >= PKG.population.start * C.populationMultipleByYear50,
    `${(y50.pop / PKG.population.start).toFixed(2)}×`
  ],
  [
    `Зданий не меньше ${C.buildingsByYear50} к ${Math.min(50, YEARS)} году`,
    y50.buildings >= C.buildingsByYear50,
    `${y50.buildings}`
  ],
  [
    `Ни один ресурс не голодал дольше ${C.noResourceStarvedLongerThanDays} дней`,
    worstStarve <= C.noResourceStarvedLongerThanDays,
    `максимум ${worstStarve} дней`
  ]
];
let failed = 0;
