...
import { allBuildCostResourcesAreSinks, resourceIndexOf, validateStartPackage } from "@galaxy-sim/sim-core";
    ...
    for (const run of runs) {
        ...
        if (result.exitCode !== 0) {
            if (run.allowKnownOpenIssue === true && isKnownBootstrapOpenIssue(result.output)) {
                console.warn(`${run.name}: known open balance issue preserved in report; continuing infrastructure validation.`);
            }
            else {
...
function checkBaseline(validateOutput, techOutput, baseline) {
    let ok = true;
    ...
    if (materialCycles !== baseline.materialCycles) {
        console.error(`baseline: material cycles expected ${baseline.materialCycles}, got ${materialCycles}`);
        ok = false;
    }
    if (energyCycles < baseline.energyCyclesMin) {
        console.error(`baseline: expected at least ${baseline.energyCyclesMin} energy cycles, got ${energyCycles}`);
        ok = false;
    }
    const rareRatios = Array.from(validateOutput.matchAll(/= ×([0-9]+(?:\.[0-9]+)?) от обычного/gu)).map((match) => Number(match[1]));
    for (const ratio of rareRatios) {
        if (ratio < baseline.rareRawRatio.min || ratio > baseline.rareRawRatio.max) {
            console.error(`baseline: rare/common ratio ${ratio} outside ${baseline.rareRawRatio.min}..${baseline.rareRawRatio.max}`);
            ok = false;
        }
    }
    const balances = /Итого по фазе 1: физика \d+ \((\d+)%\), инженерия \d+ \((\d+)%\), биология \d+ \((\d+)%\)/u.exec(techOutput);
    if (balances === null) {
    ...
    if (baseline.energyCostShareMin > 0 &&
...
async function checkStageTwoRuntimeData(baseline) {
    const data = await loadStageTwoData();
    let ok = true;
    if (data.resources.length < 35 || data.batchRecipes.length < 33 || data.continuous.length < 3) {
        console.error(`stage-two-data: incomplete graph resources=${data.resources.length}, batch=${data.batchRecipes.length}, continuous=${data.continuous.length}`);
        ok = false;
    }
    if (data.graph.materialCycleCount !== baseline.materialCycles) {
        console.error(`stage-two-data: material cycles expected ${baseline.materialCycles}, got ${data.graph.materialCycleCount}`);
        ok = false;
    }
    if (data.graph.energyCycleCount < baseline.energyCyclesMin) {
        console.error(`stage-two-data: expected at least ${baseline.energyCyclesMin} energy cycles, got ${data.graph.energyCycleCount}`);
        ok = false;
    }
    if (data.graph.energyCostShareMin + 1e-9 < baseline.energyCostShareMin) {
        console.error(`stage-two-data: energy cost share expected at least ${baseline.energyCostShareMin}, got ${data.graph.energyCostShareMin}`);
        ok = false;
    }
    ...
    if (ok)
        console.log("stage-two-data: ok");
    return ok;
}
function checkPhaseOneTurnover(data) {
    let ok = true;
    for (let resource = 0; resource < data.resources.length; resource += 1) {
        const item = data.resources[resource];
        if (item === undefined || item.phase !== 1)
            continue;
...
async function compareChainGraph() {
...
function isKnownBootstrapOpenIssue(output) {
