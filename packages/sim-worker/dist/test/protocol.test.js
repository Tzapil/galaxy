...
describe("worker protocol", () => {
    it("has typed command and response messages", () => {
        const command = { type: "init", seed: 20260904 };
        const message = {
            channel: "control",
            type: "ready",
            tick: 0,
    ...
    it("degrades by slowing game time instead of skipping ticks", () => {
        ...
        if (stats?.type === "stats")
            expect(stats.stats.actualSpeed).toBeCloseTo(100);
    });
    it("save and load restore a transferable snapshot buffer", () => {
        ...
        if (saved?.type !== "saved")
            throw new Error("Runtime did not return a save buffer.");
    ...
    it("render snapshots are flat ArrayBuffers honoring slice subscriptions", () => {
        ...
        if (snapshot?.type !== "snapshot")
            throw new Error("Runtime did not return a render snapshot.");
