...
describe("stage one world model", () => {
  ...
  it("keeps the test gate graph connected and starts factions far apart", () => {
    ...
    expect(
      routes.find(sim.world.systems, sim.world.gates, firstCapital, secondCapital).jumps
  ...
  it("uses real resource ids for the slice resources", () => {
    ...
    expect(data.sliceResourceIndices.map((resource) => data.resources[resource]?.id)).toEqual(
      expectedIds
    );
    for (let i = 0; i < expectedIds.length; i += 1) {
      expect(resourceIndexOf(data.resourceIndex, expectedIds[i] ?? "")).toBe(
        data.sliceResourceIndices[i]
      );
    }
  });
