...
describe("stage one market and logistics", () => {
  ...
  it("reserves a single profitable job for only one of twenty idle haulers", () => {
    ...
    for (let i = 0; i < 20; i += 1) world.addHauler(0, 0, 1600, 220, 7);

    let launched = 0;
    for (let ship = 0; ship < world.ships.length; ship += 1) {
      if (launchBestLocalJob(data, world, jobs, routes, queue, ship, 0) === 1) launched += 1;
    }
  ...
  it("lets a hauler leave a fuel-less destination using its own tank", () => {
    ...
    expect(
      launchBestLocalJob(data, world, oneFoodJob(data, world, 0, 1), routes, queue, ship, 0)
    ).toBe(1);
    ...
    expect(
      launchBestLocalJob(data, world, oneFoodJob(data, world, 1, 0), routes, queue, ship, arrival)
    ).toBe(1);
...
function twoColonyWorld(data: StageOneData): StageOneWorld {
  const world = StageOneWorld.create(data);
  world.systems.add(0, 0, 0, -1);
  world.systems.add(1, 0, 0, -1);
