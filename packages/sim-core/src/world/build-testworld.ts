import { buildingIndexOf, resourceIndexOf, type StageOneData } from "../stage-one/data.js";
...
export function buildTestWorld(data: StageOneData, seed: number): StageOneWorld {
  ...
  for (let system = 0; system < SYSTEM_COUNT; system += 1) {
    ...
    const planet = world.addBody(
      system,
      BodyType.Planet,
      0.8 + rng.nextInt(0, 70) / 100,
      habitability,
      18 + rng.nextInt(0, 5),
      -1,
      0
    ...
    if (system % 2 === 0) world.bodies.addDeposit(planet, ore, 0.7);
  ...
  for (let i = 0; i < 2; i += 1) world.addHauler(factionA, 0, 1600, 220, 7);
  for (let i = 0; i < 2; i += 1) world.addHauler(factionA, 1, 1600, 220, 7);
  for (let i = 0; i < 2; i += 1) world.addHauler(factionB, 10, 1600, 220, 7);
  for (let i = 0; i < 2; i += 1) world.addHauler(factionB, 11, 1600, 220, 7);
...
function seedColony(
  ...
  for (let i = 0; i < buildings.length; i += 1) {
    world.buildings.addBuilt(
      ...
      world.bodies,
      body,
      buildingIndexOf(data.buildingIndex, buildings[i] ?? "")
    );
  }
