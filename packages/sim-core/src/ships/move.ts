...
import { type GovernmentContracts, payContractSubsidy } from "../treasury/contracts.js";
...
export const enum LaunchResult {
  Launched = 1,
  NoJob = 2,
  NoFuel = 3,
  NoCargo = 4,
...
export function assignIdleHaulers(
  data: StageOneData,
  world: StageOneWorld,
  jobs: JobBoard,
  routes: RoutePlanner,
  queue: EventQueue,
  tick: number
): { readonly launched: number; readonly failedFuel: number } {
  let launched = 0;
  let failedFuel = 0;
  for (let ship = 0; ship < world.ships.length; ship += 1) {
    ...
    if (result === LaunchResult.Launched) launched += 1;
    if (result === LaunchResult.NoFuel) failedFuel += 1;
  }
  return { launched, failedFuel };
}

export function launchBestLocalJob(
  data: StageOneData,
  world: StageOneWorld,
  jobs: JobBoard,
  routes: RoutePlanner,
  ...
  if (job < 0) return repositionToBestSource(data, world, jobs, routes, queue, ship, tick);
  ...
  if ((world.ships.fuelTank[ship] ?? 0) + 1e-9 < fuelNeed) {
    ...
    world.eventLog.append(
      tick,
      StageOneLogKind.DepartureFailedFuel,
      sourceSystem,
      sourceBody,
      ship,
      -1,
  ...
  const maxByVolume =
    (world.ships.cargoCapacity[ship] ?? 0) / Math.max(0.000001, data.unitVolume[resource] ?? 1);
  ...
  world.eventLog.append(
    tick,
    StageOneLogKind.HaulerLaunched,
    sourceSystem,
    sourceBody,
    ship,
    resource,
...
function repositionToBestSource(
  data: StageOneData,
  world: StageOneWorld,
  jobs: JobBoard,
  routes: RoutePlanner,
  ...
  if (job < 0) return LaunchResult.NoJob;
...
export function handleShipArrival(
  data: StageOneData,
  world: StageOneWorld,
  queue: EventQueue,
  ship: number,
  ...
  if (targetBody >= 0 && resource >= 0 && amount > 0) {
    ...
    world.eventLog.append(
      tick,
      StageOneLogKind.ShipmentDelivered,
...
function findFactionBodyInSystem(world: StageOneWorld, faction: number, system: number): number {
  let body = world.factions.firstColony[faction] ?? -1;
  while (body >= 0) {
    if ((world.bodies.system[body] ?? -1) === system) return body;
