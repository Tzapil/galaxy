...
export function normalizeGalaxyParams(params: GalaxyGenerationParams = {}): NormalizedGalaxyParams {
  const systemCount = integerParam("systemCount", params.systemCount ?? 500, 1, 2000);
  const galaxyRadius = finiteParam(
    "galaxyRadius",
    params.galaxyRadius ?? DEFAULT_RADIUS,
    100,
    5000
  );
  ...
  const maxGateLength =
    params.maxGateLength === undefined
      ? undefined
  ...
  const planetsMax = integerParam(
    "planetsPerSystemMax",
    params.planetsPerSystemMax ?? 9,
    planetsMin,
    16
  );
  return {
    ...
    resourceClusterStrength: finiteParam(
      "resourceClusterStrength",
      params.resourceClusterStrength ?? 0.7,
      0,
      1
    ),
    rareResourceAbundance: finiteParam(
      "rareResourceAbundance",
      params.rareResourceAbundance ?? 0.05,
      0,
      1
    ),
    ...
    startViabilityJumps: integerParam(
      "startViabilityJumps",
      params.startViabilityJumps ?? params.factionMinJumps ?? 6,
      1,
      64
    ),
...
export function paramsWithPreset(
  presets: readonly GalaxyPreset[],
  presetId: string,
  overrides: GalaxyGenerationParams = {}
): GalaxyGenerationParams {
  for (let i = 0; i < presets.length; i += 1) {
    const preset = presets[i];
    if (preset?.id === presetId) return { ...preset.params, ...overrides };
  }
...
function defaultMinimumDistance(
  systemCount: number,
  radius: number,
  shape: GalaxyShape,
...
function finiteParam(name: string, value: number, min: number, max: number): number {
...
function integerParam(name: string, value: number, min: number, max: number): number {
