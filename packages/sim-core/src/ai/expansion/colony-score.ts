...
export function scoreColonyTarget(
  data: StageOneData,
  world: StageOneWorld,
  routes: RoutePlanner,
  faction: number,
  body: number,
  bottleneckResource = -1
): ColonyScore {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (resource === bottleneckResource) score += 50 * Math.max(0.5, yieldValue);
  }
  score /= 1 + nearest / 12;
  return { body, score, bottleneckResource };
}

export function bestColonyTarget(
  data: StageOneData,
  world: StageOneWorld,
  routes: RoutePlanner,
  faction: number,
  ...
  let best: ColonyScore = { body: -1, score: Number.NEGATIVE_INFINITY, bottleneckResource };
  for (let body = 0; body < world.bodies.length; body += 1) {
    ...
    if (score.score > best.score + 1e-9) best = score;
  }
  return best;
}

function nearestFactionColonyDistance(
  world: StageOneWorld,
  routes: RoutePlanner,
  faction: number,
  targetSystem: number
  ...
  while (body >= 0) {
    ...
    if (route.reachable) nearest = Math.min(nearest, route.jumps);
    body = world.bodies.nextInFaction[body] ?? -1;
  }
  return nearest;
}

function resourceScore(data: StageOneData, resource: number, yieldValue: number): number {
  const id = data.resources[resource]?.id ?? "";
  if (id === "rare_earth" || id === "crystals" || id === "radioactives") {
    return 25 * yieldValue;
  }
  if (id === "ore") return 6 * yieldValue;
  if (id === "ice") return 8 * yieldValue;
  if (id === "gas") return 7 * yieldValue;
  return 3 * yieldValue;
}
