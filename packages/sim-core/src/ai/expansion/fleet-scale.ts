...
export function scaleCivilianFleet(
  data: StageOneData,
  world: StageOneWorld,
  jobs: JobBoard,
  faction: number,
  ...
  if (jobs.count <= idle * 4 + 6) return { built: false, ship: -1 };
  if ((world.factions.treasury[faction] ?? 0) < 1200) return { built: false, ship: -1 };
...
function removeShipKit(data: StageOneData, world: StageOneWorld, stockpile: number): boolean {
  ...
  if (hullFrames < 0 || thrusters < 0 || reactors < 0 || lifeSupport < 0) return false;
  if (!world.stockpiles.hasAtLeast(stockpile, hullFrames, 3)) return false;
  if (!world.stockpiles.hasAtLeast(stockpile, thrusters, 2)) return false;
  if (!world.stockpiles.hasAtLeast(stockpile, reactors, 1)) return false;
  if (!world.stockpiles.hasAtLeast(stockpile, lifeSupport, 2)) return false;
...
function idleHaulersForFaction(world: StageOneWorld, faction: number): number {
  let idle = 0;
...
function factionIncome(world: StageOneWorld, faction: number): number {
  let population = 0;
...
function factionUpkeep(world: StageOneWorld, faction: number): number {
  let upkeep = 0;
  for (let ship = 0; ship < world.ships.length; ship += 1) {
    if (world.ships.faction[ship] !== faction) continue;
    if (world.ships.state[ship] === ShipState.Disbanded) continue;
    if (world.ships.role[ship] === ShipRole.Warship) upkeep += 14;
    else upkeep += 2.5;
  }
  return upkeep;
}
