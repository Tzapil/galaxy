...
export const enum AiOperationalTaskKind {
  BuildProducer = 1,
  ColonizeResource = 2,
  ResearchUnlock = 3,
  Stabilize = 4
...
export function findBottleneck(
  ...
  for (let resource = 0; resource < data.resources.length; resource += 1) {
    ...
    if (demandPerDay <= 0.0001) continue;
...
export function toOperationalTask(
  data: StageOneData,
  world: StageOneWorld,
  faction: number,
...
export function reserveDays(stock: number, demandPerDay: number): number {
  if (demandPerDay <= 0) return Number.POSITIVE_INFINITY;
  return stock / demandPerDay;
}

function bestOwnedBodyForBuilding(
  data: StageOneData,
  world: StageOneWorld,
  faction: number,
  buildingType: number
...
function bestUnownedBodyForResource(
  data: StageOneData,
  world: StageOneWorld,
  faction: number,
  resource: number
  ...
  for (let body = 0; body < world.bodies.length; body += 1) {
    if ((world.bodies.owner[body] ?? -1) >= 0) continue;
    if (!bodyCanSupplyResource(data, world, body, resource)) continue;
    const distance = Math.abs((world.bodies.system[body] ?? 0) - capital);
    const score =
      (world.bodies.habitability[body] ?? 0) * 25 +
...
function bodyCanSupplyResource(
  data: StageOneData,
  world: StageOneWorld,
  body: number,
  resource: number
...
function isVital(data: StageOneData, resource: number): boolean {
  return (
