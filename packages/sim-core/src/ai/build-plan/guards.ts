...
export function guardVitalVsComfort(data: StageOneData, resource: number): NeedBranch {
  if ((data.populationNeeds.perThousandPopPerDay[resource] ?? 0) <= 0) return NeedBranch.Industrial;
...
export function guardHousingCap(
  data: StageOneData,
  world: StageOneWorld,
  body: number,
  plannedHousing = 0
...
export function guardStockHorizon(
  world: StageOneWorld,
  faction: number,
  resource: number,
  demandPerDay: number,
  ...
  if (demandPerDay <= 0) return false;
  let stock = 0;
...
export function guardDemolishByFlow(
  data: StageOneData,
  world: StageOneWorld,
  building: number
): DemolitionCheck {
...
export function guardPowerAvailable(
  data: StageOneData,
  world: StageOneWorld,
  body: number,
  buildingType: number,
  ...
  if (data.buildings[buildingType]?.powerSource === true) return true;
  if (plannedPower) return true;
...
export function guardAlternativeProducer(
  data: StageOneData,
  world: StageOneWorld,
  faction: number,
  resource: number,
  preferredBuildingType: number
): number {
  const biomass = data.resourceIndex.get("biomass") ?? -1;
  if (resource !== biomass) return preferredBuildingType;
...
function ownedPlaceableBodyExists(
  data: StageOneData,
  world: StageOneWorld,
  faction: number,
  buildingType: number
