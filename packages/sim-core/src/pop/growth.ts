...
export function updatePopulationGrowth(
  ...
  for (let body = 0; body < bodies.length; body += 1) {
    ...
    if (owner < 0 || population <= 0) continue;
...
export function populationCapacity(bodies: Bodies, body: number): number {
...
export function unrestFromSupply(vital: number): number {
  if (vital >= 1) return 0;
  if (vital >= 0.8) return lerp(0.15, 0, (vital - 0.8) / 0.2);
  if (vital >= 0.5) return lerp(0.45, 0.15, (vital - 0.5) / 0.3);
  if (vital >= 0.3) return lerp(0.8, 0.45, (vital - 0.3) / 0.2);
  return lerp(1, 0.8, vital / 0.3);
}

function minVitalReserveDays(
  data: StageOneData,
  bodies: Bodies,
  stockpiles: Stockpiles,
  body: number
