...
export const enum EconProducerKind {
  Batch = 1,
  Continuous = 2
}

export const enum EconConsumerKind {
  Batch = 1,
  Continuous = 2,
  Population = 3,
  Sink = 4
...
export interface EconGraphInput {
  ...
  readonly populationNeeds: { readonly perThousandPopPerDay: Float64Array };
  readonly energyResource: number;
}

interface ProducerEdge {
  readonly resource: number;
  readonly kind: EconProducerKind;
  readonly recipe: number;
}

interface ConsumerEdge {
  readonly resource: number;
  readonly kind: EconConsumerKind;
  readonly recipe: number;
}

export function buildEconGraph(input: EconGraphInput, baseValue: Float64Array): EconGraph {
  const producedEdges: ProducerEdge[] = [];
  const consumedEdges: ConsumerEdge[] = [];
...
export function computeBaseValues(input: EconGraphInput): Float64Array {
  const values = new Float64Array(input.resources.length);
  for (let i = 0; i < values.length; i += 1) values[i] = Number.POSITIVE_INFINITY;
...
function explodeRecursive(
  ...
  if (metadata === undefined) throw new RangeError("Resource index is invalid.");
  if (metadata.category === "raw") {
    out[resource] = (out[resource] ?? 0) + amount;
    return;
  }
  if (resource === input.energyResource || guard[resource] === 1) return;
  const best = bestProducerFor(input, values, resource);
  if (best.recipe < 0) return;
  guard[resource] = 1;
  const inputs =
    best.kind === EconProducerKind.Batch
...
function bestProducerFor(
  input: EconGraphInput,
  values: Float64Array,
  resource: number
): { readonly kind: EconProducerKind; readonly recipe: number } {
...
function computeChainDepth(input: EconGraphInput): Uint16Array {
...
function chainDepthFor(
  ...
  if (visited[resource] === 1) return depth[resource] ?? 0;
  if (visiting[resource] === 1) return 0;
  visiting[resource] = 1;
  let maxDepth = 0;
  for (let recipeIndex = 0; recipeIndex < input.batchRecipes.length; recipeIndex += 1) {
    const recipe = must(input.batchRecipes[recipeIndex], "batch recipe");
...
function countCycles(input: EconGraphInput, wantsEnergyCycle: boolean): number {
...
function computeEnergyCostShareMin(input: EconGraphInput, values: Float64Array): number {
  ...
  for (let recipeIndex = 0; recipeIndex < input.batchRecipes.length; recipeIndex += 1) {
    ...
    for (let i = 0; i < recipe.inputs.length; i += 1) {
      ...
      total += cost;
      if (item.resource === input.energyResource) energyCost += cost;
    }
    if (energyCost > 0 && total > 0) minShare = Math.min(minShare, energyCost / total);
  }
  return Number.isFinite(minShare) ? minShare : 0;
}

function pushProducers(
  edges: ProducerEdge[],
  bag: readonly ResourceAmount[],
  kind: EconProducerKind,
  recipe: number
): void {
...
function pushConsumers(
  edges: ConsumerEdge[],
  bag: readonly ResourceAmount[],
  kind: EconConsumerKind,
  recipe: number
): void {
  for (let i = 0; i < bag.length; i += 1) {
...
function startsFor(
  ...
  edges: readonly { readonly resource: number }[]
): Int32Array {
  const starts = new Int32Array(resourceCount);
  starts.fill(-1);
  for (let i = 0; i < edges.length; i += 1) {
    const resource = edges[i]?.resource ?? -1;
    if (resource >= 0 && (starts[resource] ?? -1) < 0) starts[resource] = i;
  }
  return starts;
}
...
function countsFor(
  ...
  for (let i = 0; i < edges.length; i += 1) {
    const resource = edges[i]?.resource ?? -1;
    if (resource >= 0) counts[resource] = (counts[resource] ?? 0) + 1;
  }
  return counts;
}

function edgeOrder(
  a: { readonly resource: number; readonly recipe: number; readonly kind: number },
  b: { readonly resource: number; readonly recipe: number; readonly kind: number }
): number {
...
function bagHasResource(bag: readonly ResourceAmount[], resource: number): boolean {
  for (let i = 0; i < bag.length; i += 1) {
    if ((bag[i]?.resource ?? -1) === resource) return true;
  }
  return false;
}

function bagAmount(bag: readonly ResourceAmount[], resource: number): number {
  for (let i = 0; i < bag.length; i += 1) {
    const item = bag[i];
    if (item?.resource === resource) return item.amount;
  }
  return 0;
}

function stackContains(stack: Int32Array, depth: number, resource: number): boolean {
  for (let i = 0; i <= depth; i += 1) {
    if (stack[i] === resource) return true;
  }
  return false;
}
