...
export function processContinuousBuildings(data: StageOneData, world: StageOneWorld): void {
  ...
  for (let building = 0; building < buildings.length; building += 1) {
    ...
    if (state === BuildingState.UnderConstruction || state === BuildingState.Demolished) continue;
    const processIndex = buildings.continuousProcess[building] ?? -1;
    if (processIndex < 0) continue;
    if (!hasWorkersForBuilding(world.bodies, buildings, building)) continue;
    const process = data.continuous[processIndex];
    if (process === undefined) throw new RangeError("Continuous process index is invalid.");
    ...
    for (let i = 0; i < process.outputsPerTick.length; i += 1) {
      ...
      if (output === undefined) throw new RangeError("Continuous output is inconsistent.");
      const multiplier =
        output.resource === data.energyResource && faction >= 0
