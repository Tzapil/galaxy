...
import type { ArenaColumnSnapshot } from "../soa/arena.js";
...
export function writeStateSnapshot(state: SnapshotState): ArrayBuffer {
  const eventQueueBuffer = state.eventQueue.serialize();
  const byteLength =
    28 + rngStreamsByteLength(state) + 4 + eventQueueBuffer.byteLength + arenasByteLength(state);
...
function rngStreamsByteLength(state: SnapshotState): number {
  let bytes = 0;
  for (const stream of state.rngStreams) bytes += 2 + stream.name.length + 24;
  return bytes;
}

function arenasByteLength(state: SnapshotState): number {
  let bytes = 0;
  for (const arena of state.arenas) {
...
function writeString(view: DataView, target: Uint8Array, offset: number, value: string): number {
  if (value.length > 0xffff) throw new RangeError("Snapshot string is too long.");
  view.setUint16(offset, value.length, true);
  let nextOffset = offset + 2;
  for (let i = 0; i < value.length; i += 1) {
    const code = value.charCodeAt(i);
    if (code > 0x7f) throw new RangeError("Snapshot metadata must be ASCII.");
