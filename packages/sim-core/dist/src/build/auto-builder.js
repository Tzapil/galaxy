...
export function runAutoBuilder(data, world, queue, tick) {
    let started = 0;
    for (let faction = 0; faction < world.factions.length; faction += 1) {
        if (activeConstructionForFaction(world, faction) > 0)
            continue;
        const plan = selectBuildingForFaction(data, world, faction);
        if (plan.body < 0 || plan.building < 0)
            continue;
...
function selectBuildingForFaction(data, world, faction) {
...
function activeConstructionForFaction(world, faction) {
    let active = 0;
    for (let building = 0; building < world.buildings.length; building += 1) {
        ...
        if (body >= 0 && (world.bodies.owner[body] ?? -1) === faction)
            active += 1;
    }
    return active;
}
function firstIdleNoPowerBody(world, faction) {
    for (let building = 0; building < world.buildings.length; building += 1) {
        if (world.buildings.state[building] !== BuildingState.IdleNoPower)
            continue;
        const body = world.buildings.body[building] ?? -1;
        if (body >= 0 && (world.bodies.owner[body] ?? -1) === faction)
            return body;
...
function mostPressingVitalShortage(data, world, faction) {
    let bestResource = -1;
    let bestSupply = 1;
...
function strategicInputCandidate(data, world, faction, vitalResource) {
    ...
    if (biomass >= 0 && (vitalResource === food || vitalResource === medicine)) {
        const dailyBiomass = (food >= 0 ? factionDailyNeed(data, world, faction, food) * (18 / 24) : 0) +
            (medicine >= 0 ? factionDailyNeed(data, world, faction, medicine) * (6 / 9) : 0);
        if (factionReserveDaysForUse(world, faction, biomass, dailyBiomass) < VITAL_RESERVE_TARGET_DAYS) {
            ...
            if (hydroponics.body >= 0)
                return hydroponics;
        }
    }
    if (medicine >= 0 && polymers >= 0 && vitalResource === medicine) {
        const dailyPolymers = factionDailyNeed(data, world, faction, medicine) * (2 / 9);
        if (factionReserveDaysForUse(world, faction, polymers, dailyPolymers) < VITAL_RESERVE_TARGET_DAYS) {
            const polymerPlant = findPlaceableBody(data, world, faction, "polymer_plant");
            if (polymerPlant.body >= 0)
                return polymerPlant;
        }
    }
    return { body: -1, building: -1 };
}
function producerCandidateForVital(data, world, faction, resource) {
    for (const [resourceId, buildingId] of vitalProducerOrder) {
        if (resourceIndexOf(data.resourceIndex, resourceId) !== resource)
            continue;
        return findPlaceableBody(data, world, faction, buildingId);
    }
    return { body: -1, building: -1 };
}
function powerCandidateForBody(data, world, body) {
    const building = data.buildingIndex.get("solar_array") ?? -1;
    if (building < 0)
        return { body: -1, building: -1 };
    if (!belowCopyLimit(data, world, body, building))
        return { body: -1, building: -1 };
    if (!hasWorkforceHeadroom(data, world, body, building))
        return { body: -1, building: -1 };
    if (!validatePlacement(data, world, body, building).ok)
        return { body: -1, building: -1 };
    return { body, building };
}
function housingCandidate(data, world, faction) {
...
function populationCapacity(world, body) {
...
function findPlaceableBody(data, world, faction, buildingId) {
...
function hasWorkforceHeadroom(data, world, body, buildingType) {
...
function workerLoadOnBody(world, body) {
    let load = 0;
    let building = world.bodies.firstBuilding[body] ?? -1;
    while (building >= 0) {
...
function vitalReservesAbove(data, world, faction, days) {
    for (let resource = 0; resource < data.resources.length; resource += 1) {
        if ((data.populationNeeds.perThousandPopPerDay[resource] ?? 0) <= 0)
            continue;
        if (data.populationNeeds.comfortOnly[resource] === 1)
            continue;
        if (factionReserveDays(data, world, faction, resource) < days)
            return false;
    }
    return true;
}
function factionReserveDays(data, world, faction, resource) {
    return factionReserveDaysForUse(world, faction, resource, factionDailyNeed(data, world, faction, resource));
}
function factionReserveDaysForUse(world, faction, resource, dailyUse) {
    if (dailyUse <= 0)
...
function factionDailyNeed(data, world, faction, resource) {
    let demand = 0;
    let body = world.factions.firstColony[faction] ?? -1;
    while (body >= 0) {
        const population = world.bodies.population[body] ?? 0;
        const baseRate = data.populationNeeds.perThousandPopPerDay[resource] ?? 0;
        const rate = data.populationNeeds.comfortOnly[resource] === 1
