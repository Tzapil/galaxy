import { AiLayer, AiScheduler } from "../ai/scheduler.js";
import { findBottleneck, toOperationalTask, AiOperationalTaskKind } from "../ai/bottleneck.js";
...
export class StageTwoSimulation {
    ...
    applyEvents(count, instrumentation) {
        for (let i = 0; i < count; i += 1) {
            ...
            if (kind === EventKind.BatchComplete) {
                ...
                else if (handleShipArrival(this.data, this.world, this.queue, payload, this.tick, this.contracts)) {
    ...
    runAi(instrumentation) {
        ...
        for (let i = 0; i < this.aiRuns.length; i += 1) {
            ...
            if (run?.layer === AiLayer.Strategic)
                this.runStrategicAi(run.faction);
        }
        ...
        for (let i = 0; i < this.aiRuns.length; i += 1) {
            const run = this.aiRuns[i];
            if (run?.layer === AiLayer.Operational)
                this.runOperationalAi(run.faction);
        }
        instrumentation?.end(InstrumentSubsystem.AiOperational, operationalStarted);
    }
    runStrategicAi(faction) {
        ...
        logStrategicGoal(this.data, this.world, this.tick, faction, goal.subject, resource, bottleneck?.deficitPerDay ?? goal.priority);
    ...
    runOperationalAi(faction) {
        const weights = personalityWeightsForFaction(this.data, this.world, faction);
        const goal = createStrategicGoal(this.data, this.world, faction, this.tick, weights, this.lastBottleneckResource[faction] ?? -1);
        const bottleneck = findBottleneck(this.data, this.world, faction, goal);
        if (bottleneck !== undefined) {
            this.aiOperations += bottleneck.operations;
            if ((this.lastBottleneckResource[faction] ?? -1) !== bottleneck.resource) {
                logBottleneck(this.data, this.world, this.tick, faction, bottleneck.resource, bottleneck.deficitPerDay);
            }
        ...
        if (task.kind === AiOperationalTaskKind.BuildProducer &&
        ...
        else if (task.kind === AiOperationalTaskKind.ColonizeResource &&
            this.canStartAiExpansion(faction)) {
            const colonization = runColonization(this.data, this.world, this.routes, this.queue, faction, this.tick, task.resource);
            if (colonization.launched)
                this.aiColonizationLaunches += 1;
            if (colonization.built)
                this.aiOperations += 1;
        }
        const fleet = scaleCivilianFleet(this.data, this.world, this.jobs, faction, this.tick);
        if (fleet.built)
            this.aiFleetBuilds += 1;
        this.runFleetIndustryNudge(faction);
        this.runExpansionNudge(faction);
        this.aiOperationalDecisions += 1;
    }
    runFleetIndustryNudge(faction) {
        if (this.tick < 365 * 900)
            return;
        const task = this.fleetIndustryTask(faction);
        if (task === undefined || !this.canStartFleetIndustry(faction))
            return;
        const plan = {
            faction,
            items: [
    ...
    fleetIndustryTask(faction) {
        ...
        if (hullFrames < 0 || hullYard < 0 || shipyard < 0)
            return undefined;
    ...
    canStartAiBuild(faction, resource) {
        if (this.resourceIsStabilityCritical(resource))
            return true;
        if ((this.world.factions.treasury[faction] ?? 0) < 500)
            return false;
        return this.minVitalReserveDays(faction) >= 90;
    }
    canStartAiExpansion(faction) {
        if ((this.world.factions.treasury[faction] ?? 0) < 20_000)
            return false;
        if (this.maxTrackedZeroStreak() > 0)
            return false;
        if (this.minVitalReserveDays(faction) < 14)
            return false;
        const population = this.factionPopulation(faction);
        return population > 200;
    }
    runExpansionNudge(faction) {
        if (this.tick < 365 * 8 || !this.canStartAiExpansion(faction))
            return;
        const colonization = runColonization(this.data, this.world, this.routes, this.queue, faction, this.tick, this.lastBottleneckResource[faction] ?? -1);
        if (colonization.launched)
            this.aiColonizationLaunches += 1;
        if (colonization.built)
            this.aiOperations += 1;
    }
    canStartFleetIndustry(faction) {
        if ((this.world.factions.treasury[faction] ?? 0) < 50_000)
            return false;
        if (this.maxTrackedZeroStreak() > 0)
            return false;
        if (this.factionPopulation(faction) < 300)
            return false;
    ...
    addConstructionMaterialContracts() {
        for (let building = 0; building < this.world.buildings.length; building += 1) {
            if (this.world.buildings.state[building] !== BuildingState.UnderConstruction)
                continue;
            if ((this.world.buildings.finishTick[building] ?? -1) >= 0)
                continue;
            const resource = this.world.buildings.stateResource[building] ?? -1;
            if (resource < 0 || this.data.transportable[resource] !== 1)
                continue;
    ...
    addResearchConversionInputContracts() {
        for (let faction = 0; faction < this.world.factions.length; faction += 1) {
            ...
            if (capital < 0)
                continue;
            const cost = repeatableCostAtLevel(tech, (this.world.techState.level(current, faction) ?? 0) + 1);
            this.addScienceInputContracts(faction, capital, "data_physics", cost.physics - (this.world.techState.progressPhysics[faction] ?? 0));
            this.addScienceInputContracts(faction, capital, "data_engineering", cost.engineering - (this.world.techState.progressEngineering[faction] ?? 0));
            this.addScienceInputContracts(faction, capital, "data_bio", cost.bio - (this.world.techState.progressBio[faction] ?? 0));
        }
    }
    addScienceInputContracts(faction, capital, outputResourceId, remainingOutput) {
        if (remainingOutput <= 0.001)
            return;
            ...
            if (state !== BuildingState.Demolished && state !== BuildingState.UnderConstruction) {
                ...
                if (recipe !== undefined && recipeOutputsResource(recipe, outputResource)) {
                    for (let i = 0; i < recipe.inputs.length; i += 1) {
                        ...
                        if (input.resource === this.data.energyResource)
                            continue;
                        if (this.data.transportable[input.resource] !== 1)
                            continue;
                        this.contracts.add({
                            faction,
                            targetBody: capital,
    ...
    addProducerInputContracts(faction, body, outputResource) {
    ...
    resourceIsStabilityCritical(resource) {
        if (resource < 0)
            return false;
        if (resource === this.data.energyResource)
            return true;
        const id = this.data.resources[resource]?.id;
        if (id === "fuel" || id === "ice" || id === "biomass" || id === "gas" || id === "polymers") {
            return true;
        }
        return ((this.data.populationNeeds.perThousandPopPerDay[resource] ?? 0) > 0 &&
            this.data.populationNeeds.comfortOnly[resource] !== 1);
    }
    factionHasBuilding(faction, buildingType) {
        for (let building = 0; building < this.world.buildings.length; building += 1) {
            const body = this.world.buildings.body[building] ?? -1;
            if ((this.world.bodies.owner[body] ?? -1) !== faction)
                continue;
            if (this.world.buildings.type[building] === buildingType &&
    ...
    bestBodyForBuilding(faction, buildingType) {
        let bestBody = -1;
    ...
    minVitalReserveDays(faction) {
        let min = Number.POSITIVE_INFINITY;
        for (let resource = 0; resource < this.data.resources.length; resource += 1) {
            if ((this.data.populationNeeds.perThousandPopPerDay[resource] ?? 0) <= 0)
                continue;
            if (this.data.populationNeeds.comfortOnly[resource] === 1)
                continue;
    ...
    factionDailyNeed(faction, resource) {
        let demand = 0;
        let body = this.world.factions.firstColony[faction] ?? -1;
        while (body >= 0) {
            demand +=
...
function contractCreditsPerUnit(data, resource) {
...
function recipeOutputsResource(recipe, resource) {
