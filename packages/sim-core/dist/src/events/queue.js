...
export class EventBatch {
    ...
    push(tick, sequenceId, kind, payloadIndex) {
        if (this.count >= this.capacity)
            throw new RangeError("EventBatch capacity exceeded.");
...
export class EventQueue {
    ...
    schedule(tick, kind, payloadIndex) {
    ...
    drainUntil(tick, out) {
        ...
        while (this.heapSize > 0) {
            ...
            if ((this.ticks[slot] ?? 0) > tick)
                break;
            this.heapPop();
            out.push(this.ticks[slot] ?? 0, this.sequenceIds[slot] ?? 0, this.kinds[slot], this.payloadIndices[slot] ?? 0);
            this.active[slot] = 0;
            this.releaseSlot(slot);
        }
        return out.count;
    ...
    static deserialize(buffer) {
        ...
        offset += 4;
        if (version !== 1)
            throw new Error(`Unsupported event queue snapshot version ${version}.`);
    ...
    isLiveHandle(handle) {
        if (handle.slot < 0 || handle.slot >= this.nextSlot)
            return false;
        return (this.active[handle.slot] === 1 && (this.generations[handle.slot] ?? 0) === handle.generation);
    }
    allocSlot() {
        let slot;
        if (this.freeCount > 0) {
    ...
    heapPush(slot) {
        ...
        while (child > 0) {
            ...
            if (this.compareSlots(parentSlot, slot) <= 0)
                break;
    ...
    heapSink(index) {
        ...
        while (true) {
            ...
            if (this.compareSlots(slot, bestSlot) <= 0)
                break;
    ...
    ensureHeapCapacity(required) {
        if (required <= this.heap.length)
            return;
    ...
    ensureFreeCapacity(required) {
        if (required <= this.freeSlots.length)
            return;
