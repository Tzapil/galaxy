...
export function applyChokepoints(systemCount, edges, regionOfSystem, chokepointStrength) {
    if (chokepointStrength <= 0)
        return edges.slice().sort(compareEdges);
    const active = edges
        .slice()
        .sort(compareEdges)
    ...
    for (let g = 0; g < groupKeys.length; g += 1) {
        ...
        if (group === undefined || group.length <= 1)
            continue;
        const keepCount = chokepointStrength >= 1 ? 1 : Math.max(1, Math.ceil(group.length * (1 - chokepointStrength)));
...
function findActiveEdge(active, edge) {
    for (let i = 0; i < active.length; i += 1) {
        const item = active[i];
        if (item?.active === true && item.edge.a === edge.a && item.edge.b === edge.b)
            return i;
    }
    return -1;
}
function materialize(active) {
    const edges = [];
    for (let i = 0; i < active.length; i += 1) {
        const item = active[i];
        if (item?.active === true)
...
//# sourceMappingURL=chokepoints.js.map