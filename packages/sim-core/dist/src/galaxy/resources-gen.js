...
const RARE_RESOURCE_IDS = ["rare_earth", "crystals", "radioactives"];
const COMMON_RESOURCE_IDS = ["ore", "ice", "gas", "silicates"];
export function createSystemResourceMap(data, points, params, rng) {
    const resources = [];
    for (let i = 0; i < points.length; i += 1)
        resources.push(new Uint8Array(data.resources.length));
    for (let i = 0; i < COMMON_RESOURCE_IDS.length; i += 1) {
        ...
        const flags = selectResourceSystems(points, params, rng, commonAbundance(id), Math.max(4, params.regionCount), i + 11);
        writeResourceFlags(resources, resource, flags);
    }
    for (let i = 0; i < RARE_RESOURCE_IDS.length; i += 1) {
        const id = RARE_RESOURCE_IDS[i] ?? "";
        const resource = resourceIndexOf(data.resourceIndex, id);
        const abundance = Math.max(params.rareResourceAbundance, (params.rareResourceClusterMin * 2) / Math.max(1, points.length));
        const flags = selectResourceSystems(points, params, rng, abundance, params.rareResourceClusterMin, i + 101);
        writeResourceFlags(resources, resource, flags);
    }
    return resources;
}
export function rareResourceIndices(data) {
    return RARE_RESOURCE_IDS.map((id) => resourceIndexOf(data.resourceIndex, id));
}
export function tierOneStartResourceIndices(data) {
    const blockedRare = new Set(RARE_RESOURCE_IDS);
    const result = [];
    for (let resource = 0; resource < data.resources.length; resource += 1) {
        const item = data.resources[resource];
        if (item === undefined)
            continue;
        if (item.phase !== 1 || item.tier !== 1 || item.category !== "raw")
            continue;
        if (blockedRare.has(item.id) || item.id === "exotic_matter")
            continue;
        result.push(resource);
    }
    return result.sort((a, b) => a - b);
}
export function rareResourceClusterCount(systemResources, edges, resource) {
    ...
    let clusters = 0;
    for (let system = 0; system < systemResources.length; system += 1) {
        if (marked[system] !== 1 || visited[system] === 1)
            continue;
        clusters += 1;
...
export function neighborResourceCorrelation(systemResources, edges, resource) {
    let both = 0;
    let either = 0;
...
function selectResourceSystems(points, params, rng, abundance, clusterCount, salt) {
...
function chooseClusterSeeds(points, count, radius, rng) {
    const seeds = [];
    if (points.length === 0)
        return seeds;
    seeds.push({
...
function nearestSeedDistance(points, system, seeds) {
...
function clusterInfluence(point, points, seeds) {
    let best = 0;
    for (let i = 0; i < seeds.length; i += 1) {
        const seed = seeds[i];
        if (seed === undefined)
...
function writeResourceFlags(resources, resource, flags) {
...
function markResourceCluster(start, marked, visited, edges) {
...
function stableNoise(value, salt) {
...
//# sourceMappingURL=resources-gen.js.map