const TAU = Math.PI * 2;
export function createGalaxyShapeSampler(params, rng) {
    if (params.shape === "disc")
        return discSampler(params.galaxyRadius);
    if (params.shape === "spiral")
        return spiralSampler(params.galaxyRadius, params);
    if (params.shape === "ring")
        return ringSampler(params.galaxyRadius);
    return clusterSampler(params.galaxyRadius, rng);
}
function discSampler(radius) {
    return {
        sample: (rng) => {
...
function spiralSampler(radius, params) {
...
function createClusterCenters(radius, rng) {
    const centers = [];
    const count = 7;
...
function pointFromPolar(angle, distance) {
    return {
        x: Math.cos(angle) * distance,
        y: Math.sin(angle) * distance
    };
