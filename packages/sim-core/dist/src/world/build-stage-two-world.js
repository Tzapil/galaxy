import { addStartPackageShips, applyStartPackage, addDepositsForFeatures } from "../bootstrap/start-package.js";
import { Rng } from "../rng.js";
import { featureMaskFromNames } from "../stage-one/data.js";
...
export function buildStageTwoWorld(data, seed) {
...
function addGeneratedBodies(data, world, rng, system) {
    const habitable = rng.nextInt(0, 100) < 35;
    addGeneratedBody(data, world, rng, system, BodyType.Planet, habitable ? ["habitable", "ice_deposit"] : ["ore_deposit", "silicate_deposit"], habitable ? 0.65 + rng.nextInt(0, 25) / 100 : 0.12 + rng.nextInt(0, 30) / 100);
    ...
    const rareFeatures = ["ore_deposit"];
    if (rng.nextInt(0, 100) < 45)
        rareFeatures.push("rare_earth_vein");
    if (rng.nextInt(0, 100) < 35)
        rareFeatures.push("crystal_vein");
    if (rng.nextInt(0, 100) < 30)
        rareFeatures.push("radioactive_vein");
    addGeneratedBody(data, world, rng, system, BodyType.AsteroidBelt, rareFeatures, 0);
}
function addGeneratedBody(data, world, rng, system, type, features, habitability) {
    const size = type === BodyType.AsteroidBelt
...
//# sourceMappingURL=build-stage-two-world.js.map