export function solveLinearProgram(program) {
...
function chooseEntering(tableau, objectiveRow, variables) {
    for (let col = 0; col < variables; col += 1) {
        if ((tableau[objectiveRow + col] ?? 0) < -1e-9)
            return col;
    }
    return -1;
}
function chooseLeaving(tableau, width, constraints, entering) {
    let bestRow = -1;
...
function pivot(tableau, width, height, pivotRow, pivotCol) {
...
function basicRowFor(tableau, width, constraints, col) {
    let rowWithOne = -1;
    for (let row = 0; row < constraints; row += 1) {
...
function infeasible(variables) {
    return {
        feasible: false,
        objectiveValue: 0,
        values: new Array(variables).fill(0),
        iterations: 0
    };
}
