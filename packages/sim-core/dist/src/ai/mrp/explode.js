export function explodeDemand(data, targets, options = {}) {
    ...
    for (let i = 0; i < targets.length; i += 1) {
        ...
        if (target.kind === "resource") {
            if (target.amountPerDay <= 0)
                continue;
            directPerDay[target.resource] = (directPerDay[target.resource] ?? 0) + target.amountPerDay;
            operations += explodeResource(data, target.resource, target.amountPerDay, requiredPerDay, trace, 0, maxDepth);
        }
        else {
            const hull = data.hulls[target.hull];
            if (hull === undefined || target.count <= 0 || target.horizonDays <= 0)
                continue;
            ...
            for (let item = 0; item < hull.buildRecipe.length; item += 1) {
                ...
                if (cost === undefined)
                    throw new RangeError("Hull build recipe is inconsistent.");
                const amountPerDay = cost.amount * multiplier;
                directPerDay[cost.resource] = (directPerDay[cost.resource] ?? 0) + amountPerDay;
                operations += explodeResource(data, cost.resource, amountPerDay, requiredPerDay, trace, 0, maxDepth);
            }
        }
    }
    return { requiredPerDay, directPerDay, trace, operations, maxDepth };
}
function explodeResource(data, resource, amountPerDay, requiredPerDay, trace, depth, maxDepth) {
    if (amountPerDay <= 0)
        return 0;
    requiredPerDay[resource] = (requiredPerDay[resource] ?? 0) + amountPerDay;
    trace.push({ resource, depth, amountPerDay });
    if (depth >= maxDepth || resource === data.energyResource)
        return 1;
    ...
    for (let i = 0; i < recipe.inputs.length; i += 1) {
        ...
        if (input === undefined)
            throw new RangeError("Recipe input bag is inconsistent.");
        const inputRate = (input.amount / Math.max(1, recipe.durationTicks)) * runsPerDay;
        operations += explodeResource(data, input.resource, inputRate, requiredPerDay, trace, depth + 1, maxDepth);
    }
    return operations;
}
export function chooseProducer(data, resource) {
    let best;
    let bestCost = Number.POSITIVE_INFINITY;
    for (let i = 0; i < data.batchRecipes.length; i += 1) {
        const recipe = data.batchRecipes[i];
        if (recipe === undefined || outputRateFor(recipe, resource) <= 0)
            continue;
        const unitCost = recipeUnitCost(data, recipe, resource);
        if (unitCost < bestCost - 1e-9 ||
...
function recipeUnitCost(data, recipe, resource) {
    let inputValue = 0;
    for (let i = 0; i < recipe.inputs.length; i += 1) {
        const input = recipe.inputs[i];
        if (input === undefined)
...
function outputRateFor(recipe, resource) {
    return outputAmountFor(recipe.outputs, resource) / Math.max(1, recipe.durationTicks);
}
function outputAmountFor(outputs, resource) {
