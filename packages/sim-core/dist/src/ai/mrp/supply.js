...
export function calculateFactionSupplyRates(data, world, faction) {
    ...
    for (let building = 0; building < world.buildings.length; building += 1) {
        ...
        if ((world.bodies.owner[bodyId] ?? -1) !== faction)
            continue;
...
function addBodyStockpile(data, world, body, stockpile) {
...
function addBatchOutputs(data, world, building, multiplier, productionPerDay) {
    ...
    for (let i = 0; i < recipe.outputs.length; i += 1) {
        ...
        if (output === undefined)
            throw new RangeError("Recipe output bag is inconsistent.");
...
function addContinuousOutputs(data, world, building, multiplier, productionPerDay) {
    ...
    for (let i = 0; i < process.outputsPerTick.length; i += 1) {
        ...
        if (output === undefined)
            throw new RangeError("Continuous output bag is inconsistent.");
        productionPerDay[output.resource] =
            (productionPerDay[output.resource] ?? 0) + output.amount * multiplier;
    }
}
function workerMultiplier(world, building) {
