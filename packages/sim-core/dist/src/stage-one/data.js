import { buildEconGraph, computeBaseValues } from "../econ/graph.js";
...
const PLACEMENT_ANY = PLACEMENT_PLANET | PLACEMENT_ASTEROID | PLACEMENT_GAS_GIANT | PLACEMENT_STATION | PLACEMENT_COMET;
const rawResources = [
...
export function createDefaultStageOneData() {
    ...
    const buildings = rawBuildings.map((building) => {
        ...
        const recipeWorkers = batchRecipe >= 0
...
export function createStageTwoDataFromGameData(input) {
    ...
    const resources = rawGameResources.map((resource) => ({
        id: resource.id,
        name: resource.name,
        tier: resource.tier,
    ...
    const continuous = input.recipes.continuous
        ...
        outputsPerTick: convertBagFromRecord(process.outputPerDay ?? process.outputsPerDay ?? {}, resourceIndex),
...
export function resourceIndexOf(index, id) {
    ...
    if (value === undefined)
        throw new RangeError(`Unknown resource id "${id}".`);
    return value;
}
export function buildingIndexOf(index, id) {
    const value = index.get(id);
    if (value === undefined)
...
function indexById(items) {
    ...
    for (let i = 0; i < items.length; i += 1) {
        ...
        if (map.has(item.id))
            throw new RangeError(`Duplicate id "${item.id}".`);
        map.set(item.id, i);
    }
    return map;
}
...
function convertBagFromRecord(bag, resourceIndex) {
    return Object.keys(bag)
        .map((id) => ({ resource: resourceIndexOf(resourceIndex, id), amount: bag[id] ?? 0 }))
        .sort((a, b) => a.resource - b.resource);
}
function populationNeedsFrom(continuous, resourceIndex) {
...
function powerProcessIndicesFor(continuous, energyResource) {
    const indices = [];
    for (let i = 0; i < continuous.length; i += 1) {
        if (producesResource(continuous[i], energyResource))
            indices.push(i);
    }
    return indices;
}
function producesResource(process, resource) {
    if (process === undefined)
        return false;
    for (let i = 0; i < process.outputsPerTick.length; i += 1) {
        if ((process.outputsPerTick[i]?.resource ?? -1) === resource)
            return true;
    }
    return false;
}
function buildFeatureIndex(buildings, bodies) {
    const names = [];
    for (let i = 0; i < bodies.length; i += 1) {
        for (const feature of bodies[i]?.features ?? [])
            pushFeature(names, feature);
    }
    ...
    for (let i = 0; i < names.length; i += 1) {
        if (i >= 31)
            throw new RangeError("Body feature mask supports up to 31 features.");
        map.set(names[i] ?? "", i);
    }
    return map;
}
function pushFeature(names, feature) {
    if (typeof feature !== "string") {
        for (const item of feature)
            pushFeature(names, item);
        return;
    }
    if (!names.includes(feature))
        names.push(feature);
}
function featureNamesFromIndex(index) {
    const names = new Array(index.size);
    for (const [name, bit] of index)
        names[bit] = name;
    return names;
}
function requiredFeatureMaskFrom(featureIndex, requires) {
    ...
    for (let i = 0; i < items.length; i += 1) {
        ...
        if (bit === undefined)
            throw new RangeError(`Unknown body feature "${items[i] ?? ""}".`);
        mask |= 1 << bit;
    }
    return mask;
}
export function featureMaskFromNames(featureIndex, features) {
    let mask = 0;
    for (let i = 0; i < features.length; i += 1) {
        const feature = features[i] ?? "";
        const bit = featureIndex.get(feature);
        if (bit === undefined)
            throw new RangeError(`Unknown body feature "${feature}".`);
        mask |= 1 << bit;
    }
    return mask;
}
export function bodyTypePlacementMask(type) {
    switch (type) {
...
function placementMaskFrom(values) {
    let mask = 0;
    for (let i = 0; i < values.length; i += 1) {
        const value = values[i] ?? "";
        if (value === "planet")
            mask |= PLACEMENT_PLANET | PLACEMENT_GAS_GIANT;
        else if (value === "asteroid")
            mask |= PLACEMENT_ASTEROID;
        else if (value === "station")
            mask |= PLACEMENT_STATION;
        else if (value === "comet")
            mask |= PLACEMENT_COMET;
    }
    return mask === 0 ? PLACEMENT_ANY : mask;
}
function startPackageFrom(input, buildingIndex, resourceIndex, featureIndex, hullIndex) {
...
function bodyTypeFrom(type) {
    if (type === "asteroid")
        return BodyType.AsteroidBelt;
    if (type === "station")
        return BodyType.Station;
    if (type === "comet")
...
function defaultTier(id) {
    if (id === "energy")
        return 0;
    if (id === "ore" || id === "ice" || id === "biomass" || id === "gas")
        return 1;
    if (id === "metal" || id === "water" || id === "food" || id === "fuel")
        return 2;
    return 3;
}
