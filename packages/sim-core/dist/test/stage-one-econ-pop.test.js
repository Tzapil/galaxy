...
import { detectAndBreakProductionDeadlocks, handleBatchComplete, processContinuousBuildings, tryStartBatch } from "../src/econ/batch.js";
...
import { buildingIndexOf, createDefaultStageOneData, resourceIndexOf } from "../src/stage-one/data.js";
...
describe("stage one production and population", () => {
    it("runs the ore to metal batch chain for 1000 ticks", () => {
        ...
        world.buildings.addBuilt(data, world.bodies, body, buildingIndexOf(data.buildingIndex, "solar_array"));
        world.buildings.addBuilt(data, world.bodies, body, buildingIndexOf(data.buildingIndex, "mine"));
        world.buildings.addBuilt(data, world.bodies, body, buildingIndexOf(data.buildingIndex, "smelter"));
    ...
    it("moves a building to idle missing input in the same tick", () => {
        ...
        const smelter = world.buildings.addBuilt(data, world.bodies, body, buildingIndexOf(data.buildingIndex, "smelter"));
    ...
    it("detects and breaks a direct production deadlock deterministically", () => {
        ...
        const world = singleColonyWorld(data, 50);
        const queue = new EventQueue(8);
        const mine = world.buildings.addBuilt(data, world.bodies, 0, buildingIndexOf(data.buildingIndex, "mine"));
        const smelter = world.buildings.addBuilt(data, world.bodies, 0, buildingIndexOf(data.buildingIndex, "smelter"));
    ...
    it("shrinks under starvation and recovers growth only from reserve surplus", () => {
        ...
        expect(world.bodies.population[body]).toBeLessThanOrEqual(populationCapacity(world.bodies, body));
    });
    it("keeps workforce scale compatible with colony food production", () => {
        ...
        if (recipe === undefined)
            throw new Error("Missing synth_food recipe.");
        const output = recipe.outputs.find((item) => item.resource === food);
        if (output === undefined)
            throw new Error("Missing food output.");
        expect(requiredFoodPlantsForOwnWorkforce(14, output.amount, recipe.durationTicks, 0.06)).toBeLessThan(1);
    });
...
function setSupply(world, body, resource, value) {
    const column = world.supply.emaColumns[resource];
    if (column === undefined)
        throw new RangeError("Missing supply column.");
    column[body] = value;
}
function drainBatchEvents(data, world, queue, tick) {
...
//# sourceMappingURL=stage-one-econ-pop.test.js.map