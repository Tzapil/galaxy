...
import { Rng, applyChokepoints, clusterRegions, connectedComponentCount, generateStarLayout, minimumSquaredDistance, normalizeGalaxyParams, pruneGateGraph, triangulateDelaunay } from "../src/index.js";
describe("stage three galaxy geometry", () => {
    ...
    it("keeps the poisson minimum distance at 2000 systems", () => {
        ...
        expect(Math.sqrt(minimumSquaredDistance(layout.points))).toBeGreaterThanOrEqual(layout.minSystemDistance - 1e-7);
    }, 60_000);
    it("makes the four shapes numerically distinct", () => {
        ...
        const cluster = nearestNeighborMean(generateStarLayout(11, { systemCount: 450, shape: "cluster" }).points);
        expect(ring.meanRadius).toBeGreaterThan(disc.meanRadius);
        expect(ring.radiusSpread).toBeLessThan(disc.radiusSpread);
        expect(spiralArmAlignment(spiral, 4, 0.4)).toBeLessThan(spiralArmAlignment(spiralBaseline, 4, 0.4) * 0.78);
        expect(cluster).toBeLessThan(nearestNeighborMean(generateStarLayout(12, { systemCount: 450, shape: "disc" }).points));
    }, 60_000);
    it("prunes Delaunay gates to the target average degree on 50 seeds", () => {
        for (let i = 0; i < 50; i += 1) {
            ...
            const graph = pruneGateGraph(layout.points, edges, normalizeGalaxyParams({ systemCount: 500 }), Rng.fromSeed(seed).derive("galaxy").derive("gates"));
    ...
    it("responds to sparse and dense avgGateDegree targets", () => {
        ...
        const edges = triangulateDelaunay(layout.points);
        const sparse = pruneGateGraph(layout.points, edges, normalizeGalaxyParams({ systemCount: 300, avgGateDegree: 2 }), Rng.fromSeed(99).derive("galaxy").derive("sparse-gates"));
        const dense = pruneGateGraph(layout.points, edges, normalizeGalaxyParams({ systemCount: 300, avgGateDegree: 5 }), Rng.fromSeed(99).derive("galaxy").derive("dense-gates"));
    ...
    it("keeps chokepoint pruning connected and strict at full strength", () => {
        ...
        const graph = pruneGateGraph(layout.points, candidate, normalizeGalaxyParams({ systemCount: 350, avgGateDegree: 3.2 }), Rng.fromSeed(123).derive("galaxy").derive("gates"));
        const regions = clusterRegions(layout.points, 7, Rng.fromSeed(123).derive("galaxy").derive("regions"));
...
function radialStats(points) {
    ...
    for (let i = 0; i < radii.length; i += 1)
        spread += Math.abs((radii[i] ?? 0) - mean);
    return { meanRadius: mean, radiusSpread: spread / Math.max(1, radii.length) };
}
function spiralArmAlignment(points, armCount, armTightness) {
    ...
    for (let i = 0; i < points.length; i += 1) {
        ...
        if (point === undefined)
            continue;
        const radius = Math.sqrt(point.x * point.x + point.y * point.y);
        const angle = positiveModulo(Math.atan2(point.y, point.x) - (radius / maxRadius) * twist, armWidth);
        total += Math.min(angle, armWidth - angle);
        count += 1;
    }
    return total / Math.max(1, count);
}
function maxPointRadius(points) {
    let max = 1;
    for (let i = 0; i < points.length; i += 1) {
        const point = points[i];
        if (point === undefined)
...
function nearestNeighborMean(points) {
    let total = 0;
    for (let i = 0; i < points.length; i += 1) {
...
function maxInterRegionPassages(edges, regions) {
...
//# sourceMappingURL=stage-three-geometry.test.js.map