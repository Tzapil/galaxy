...
import type { BatchRecipeRecord, GameDataFiles, GameDataSchemas, LoadedGameData } from "./types.js";
...
export function validateGameDataSchemas(data: GameDataFiles, schemas: GameDataSchemas): void {
...
export function load(data: GameDataFiles, schemas: GameDataSchemas): LoadedGameData {
  ...
  for (const recipe of data.recipes.batchRecipes) {
    ...
    for (const id of resourceIdsFromBag(recipe.inputs)) assertKnownResource(recipe.id, id, RES);
  ...
  for (const building of data.buildings.buildings) {
    if (
      ...
      throw new GameDataValidationError(
        `Building ${building.id}: recipe "${building.recipe}" does not exist.`
      );
    }
  }
  ...
  for (const cont of data.recipes.continuous) {
    for (const value of Object.values(cont)) {
      if (isQuantityBag(value)) {
        for (const id of resourceIdsFromBag(value)) assertKnownResource(cont.id, id, RES);
      }
    }
  }
  ...
  for (const sink of data.recipes.sinks) {
    if (Array.isArray(sink.consumes)) {
      for (const id of sink.consumes) assertKnownResource(sink.id, id, RES);
    }
    for (const value of Object.values(sink)) {
      if (isQuantityBag(value)) {
        for (const id of resourceIdsFromBag(value)) assertKnownResource(sink.id, id, RES);
      }
    }
  }

  for (const hull of data.hulls.hulls) {
    for (const id of resourceIdsFromBag(hull.buildRecipe)) assertKnownResource(hull.id, id, RES);
  }

  for (const mod of data.modules.modules) {
    for (const id of resourceIdsFromBag(mod.cost)) assertKnownResource(mod.id, id, RES);
  ...
  for (const doctrine of data.doctrines.doctrines) {
    for (const hullId of doctrine.hulls) {
      if (!hullById.has(hullId)) {
        throw new GameDataValidationError(
          `Doctrine ${doctrine.id}: hull "${hullId}" does not exist.`
        );
      }
    }
  ...
  for (const [id, value] of Object.entries(data.startPackage.stockpiles)) {
    if (typeof value === "number") assertKnownResource("start-package", id, RES);
  }
  for (const ship of data.startPackage.ships) {
    ...
    if (!doctrineById.has(ship.doctrine)) {
      throw new GameDataValidationError(
        `Start package: doctrine "${ship.doctrine}" does not exist.`
      );
    }
  }
  for (const id of data.startPackage.technologies) assertKnownTech("start-package", id, techById);

  const producedBy = new Map<string, readonly BatchRecipeRecord[]>();
  for (const [id, recipes] of producedByMutable) producedBy.set(id, recipes);
...
function schemaFor(key: SchemaKey, schemas: GameDataSchemas): AnySchema {
  return schemas[key] as AnySchema;
}

function dataFor(key: SchemaKey, data: GameDataFiles): unknown {
  return data[key];
}

function formatAjvErrors(key: string, errors: readonly ErrorObject[]): string {
...
function indexById<T extends { readonly id: string }>(
  ...
  for (const item of items) {
    if (map.has(item.id)) throw new GameDataValidationError(`Duplicate ${label} id "${item.id}".`);
    map.set(item.id, item);
  }
  return map;
}

function assertKnownResource(
  owner: string,
  id: string,
...
function assertKnownBuilding(
  owner: string,
  id: string,
  buildings: ReadonlyMap<string, unknown>
): void {
  if (!buildings.has(id))
    throw new GameDataValidationError(`${owner}: building "${id}" does not exist.`);
}

function assertKnownTech(owner: string, id: string, techs: ReadonlyMap<string, unknown>): void {
  if (!techs.has(id)) throw new GameDataValidationError(`${owner}: tech "${id}" does not exist.`);
}

function validateTechEffect(
  owner: string,
  effect: { readonly type: string; readonly id?: string },
  modules: ReadonlyMap<string, unknown>,
  hulls: ReadonlyMap<string, unknown>,
  buildings: ReadonlyMap<string, unknown>
): void {
  if (effect.type === "unlockModule" && (effect.id === undefined || !modules.has(effect.id))) {
    throw new GameDataValidationError(
      `${owner}: unlockModule "${effect.id ?? ""}" does not exist.`
    );
  }
  ...
  if (effect.type === "unlockBuilding" && (effect.id === undefined || !buildings.has(effect.id))) {
    throw new GameDataValidationError(
      `${owner}: unlockBuilding "${effect.id ?? ""}" does not exist.`
    );
  }
}
...
function techEffects(tech: {
  ...
  readonly effectPerLevel?: unknown;
...
function resourceIdsFromBag(bag: Record<string, number>): string[] {
  return Object.keys(bag);
}

function isQuantityBag(value: unknown): value is Record<string, number> {
