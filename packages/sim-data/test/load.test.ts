...
describe("sim-data loader", () => {
  ...
  it("links buildings, recipe resources, and producedBy references", async () => {
    ...
    for (const building of loaded.BUILDINGS) {
      if (building.recipe !== null) expect(recipeIds.has(building.recipe)).toBe(true);
    }
    expect(loaded.producedBy.get("metal")?.some((recipe) => recipe.id === "smelt_metal")).toBe(
      true
    );
  });
  ...
  it("links technology unlock effects to concrete content", async () => {
    ...
    for (const tech of loaded.data.techs.techs) {
      ...
      for (const effect of effects) {
        if (effect.type === "unlockModule") expect(modules.has(effect.id ?? "")).toBe(true);
        if (effect.type === "unlockHull") expect(hulls.has(effect.id ?? "")).toBe(true);
        if (effect.type === "unlockBuilding") expect(buildings.has(effect.id ?? "")).toBe(true);
      }
    }
  });
});
